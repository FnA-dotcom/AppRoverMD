...
public class PatientReport_old extends HttpServlet {
    ...
    void GetReportInput(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, String UserId, HttpServletResponse response) {
        Statement hstmt = null;
        ResultSet hrset = null;
        String Query = "";
        StringBuffer Clients = new StringBuffer();
        try {
            Query = "Select id,name from oe.clients where id not in (1,9,10,12)";
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(this.getServletContext()) + "Reports/PatientReport_old.html");
    ...
    void GetReport(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, String UserId, HttpServletResponse response) {
        Statement hstmt = null;
        ResultSet hrset = null;
        String Query = "";
        ...
        String Query2 = "";
        int requestedMonthInt = Integer.parseInt(request.getParameter("Month").trim());
        String clientid = "";
        String clientname = "";
        String[] months = {"January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "Novermber", "December"};
        String requestedYearString = request.getParameter("Year").trim();
        String requestedMonthString = request.getParameter("Month").trim();
        String Clients = request.getParameter("Clients").trim();
        int monthcount = 0;
        long TotalCount = 0;
        ...
        try {
            ...
            StringBuffer ClientsBuffer = new StringBuffer();
            ...
            for (int i = 1; i <= requestedMonthLength; i++) {
                List.append("<th style=\"width:02%\">" + requestedMonthString + "/" + String.format("%02d", new Object[]{Integer.valueOf(i)}) + "/" + requestedYearString + "</th> ");
            }
            ...
            while (hrset.next()) {
                ...
                monthcount = 0;
                for (int i = 1; i <= requestedMonthLength; i++) {
                    //Query2 = "SELECT COUNT(*) FROM oe.filelogs_sftp WHERE substr(dosdate,1,10)='" + requestedYearString + "-" + requestedMonthString + "-" + String.format("%02d", new Object[] { Integer.valueOf(i) }) + "' AND clientdirectory = "+clientid  ;
                    Query2 = "SELECT count(*) FROM (SELECT DISTINCT firstname, lastname,substr(dosdate,1,10) FROM filelogs_sftp " +
                            " WHERE clientdirectory= " + clientid + " AND substr(dosdate,1,10)='" + requestedYearString + "-" + requestedMonthString + "-" + String.format("%02d", new Object[]{Integer.valueOf(i)}) + "' ) t";
                    ...
                    //monthcount = monthcount+","+hrset2.getInt(1);
                    hrset2.close();
                    hstmt2.close();
                }
...
//      Parsehtm Parser = new Parsehtm(request);
//      Parser.SetField("Clients", ClientsBuffer.toString());
//      Parser.SetField("List", List.toString());
//      Parser.SetField("MonthName", " Month : "+String.format("%02d", new Object[] { Integer.valueOf(requestedMonthString) })+"");
//      Parser.GenerateHtml(out, Services.GetHtmlPath(this.getServletContext()) + "Reports/ChartCountReport.html");
