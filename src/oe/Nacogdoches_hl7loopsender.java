...
public class Nacogdoches_hl7loopsender implements Runnable {
    ...
    static {
        Nacogdoches_hl7loopsender.SignonDone = false;
    }

    public Nacogdoches_hl7loopsender(final Socket socket) {
        this.done = false;
        this.sock = socket;
        Nacogdoches_hl7loopsender.TraceNo = 0;
    }
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            final String Host = "10.57.10.15";
            System.out.println("Connecting To  Database  @  localhost ");
            conn = getConnection();
            System.out.println("Connection: " + conn);
            System.out.println("Connecting To  Epower Doc socket  @  10.57.10.15 10020");
            //Live Port
            //sock = new Socket("10.57.10.15", 10020);
            sock = new Socket("96.73.123.69", 3000);
            System.out.println("|||||||||||");
        } catch (Exception e) {
            try {
                conn.close();
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    //Query = " SELECT id,msg,mrn,flag FROM request WHERE status=0 and ClientIndex = 17 ORDER BY requestdate";
                    Query = " SELECT id,msg,mrn,flag FROM request WHERE status=0 and ClientIndex = 17 LIMIT 2";
                    ...
                    while (hrset.next()) {
                        ...
                        Thread.sleep(750L);
                        final Nacogdoches_hl7loopsender client = new Nacogdoches_hl7loopsender(sock);
                ...
                if ((dt2.getTime() - StartTime) / 1000L > 45.0) {
                    final Nacogdoches_hl7loopsender client2 = new Nacogdoches_hl7loopsender(sock);
    ...
    public static void sendmsg2(final Socket sock, String msg, final String mrn, final String flag, final Connection conn) {
        ...
        try {
            ...
            Query = "Select Title, FirstName, MiddleInitial, LastName, MaritalStatus, MRN, DOB, Age, Gender, Email, PhNumber, Address, City, State, Country,  ZipCode, SSN, Occupation, Employer, EmpContact, PriCarePhy, ReasonVisit, SelfPayChk, CreatedDate,sync,ID, " +
                    "CASE WHEN Ethnicity = 1 THEN 'Hispanic' WHEN Ethnicity = 2 THEN 'Non-Hispanic' WHEN Ethnicity = 3 THEN 'Others' ELSE 'Others' END from nacogdoches.PatientReg where MRN ='" + mrn.trim() + "'";
            ...
            if (SelfPayChk == 1) {
                ...
                if (!PriInsuranceName.equals("-")) {
                    Query = " Select PayerName from nacogdoches.ProfessionalPayers where Id =  " + PriInsuranceName;
                    //System.out.println(Query);
                    stmt = conn.createStatement();
                    rset = stmt.executeQuery(Query);
                    while (rset.next()) {
...
//                IN1 = "IN1|1|" + MemId + "||" + PriInsurance + "||||" + GrpNumber + "|||||||||||||||||||||||||||||||||||";
            ...
            Query = "Select IFNULL(NextofKinName,''), IFNULL(RelationToPatient,''), IFNULL(PhoneNumber,'') from nacogdoches.EmergencyInfo where PatientRegId = " + Id;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            Query = "Select CASE WHEN Race = 1 THEN 'Black or African American' WHEN Race = 2 THEN 'American Indian' WHEN Race = 3 THEN 'Asian' WHEN Race = 4 THEN 'Native Hawaiian or Other Specific Islander' WHEN Race = 5 THEN 'White' WHEN Race = 6 THEN 'Others' ELSE 'Others' END " +
                    "from nacogdoches.PatientReg_Details where PatientRegId = " + Id;
            //System.out.println(Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
...
//                    +"GT1|1|"+"230"+"|"+"Test^Test^M"+"||"+""+"|"+"(222)555-5698"+"^^CP^^^"+""+"||"+""+"|"+""+"\r\n"
            ...
            hstmt2 = conn.createStatement();
            hstmt2.executeUpdate("update nacogdoches.PatientReg set sync=1 where sync=0 and id=" + Id);
    ...
    private static Connection getConnection() {
        try {
            ...
            Connection connection = DriverManager.getConnection("jdbc:mysql://3.238.87.114:33308/oe?user=Dev01kolkka&password=12Wasljro402330AA1&autoReconnect=true");
//            final Connection connection = DriverManager.getConnection("jdbc:mysql://rovermd-01-master.cvsodt2nhyzz.us-east-1.rds.amazonaws.com/oe?user=rovermdadmin&password=atyu!ioujy1986");
            ...
            System.out.println("PRINT STACKTRACE" + str);
            return null;
        }
    }
...
/*    private void pushtologs(String Message)
    {
        try
        {
            String UniqueId = "";
            UniqueId = this.channels.getQueuedCustomer();
            System.out.println("CM VERSION : 1001 UID " + UniqueId + " -- " + Message);
            String FileName = "TestFile";
            FileWriter fr = new FileWriter("F:\\LOGS\\" + FileName, true);
            fr.write("VERSION : 1001  : Event Time " + new Date().toString() + " UID " + UniqueId + "  Msg    " + Message + "\r\n");
            fr.flush();
            fr.close();
