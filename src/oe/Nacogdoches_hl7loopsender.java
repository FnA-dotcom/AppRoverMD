...
//                System.out.println(PriInsuranceName + "---:PriInsurance");
...
//                IN1 = "IN1|1|||" + PriInsurance + "|^^^^|||" + GrpNumber + "||||||||||||||||||||||||||||"+MemId+"|||||||||||||"+MemId+"\r\n";
            ...
            Query = "Select IFNULL(NextofKinName,''), IFNULL(RelationToPatient,''), IFNULL(PhoneNumber,'') from " + clientdb + ".EmergencyInfo where PatientRegId = " + Id;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
...
//            pushtologs("Lower " + ee.getMessage());
            ...
            pushtologs("Exception (Nacogdoches) : " + ee.getMessage());
            pushtologs("Exception (Nacogdoches) : " + str);
            try {
/*                hstmt2 = conn.createStatement();
