...
public class Odessa_hl7loopsender implements Runnable {
    ...
    private Odessa_hl7loopsender(final Socket socket) {
        this.done = false;
        this.sock = socket;
        TraceNo = 0;
    }
...
//                IN1 = "IN1|1|||" + PriInsurance + "|^^^^|||" + GrpNumber + "||||||||||||||||||||||||||||"+MemId+"|||||||||||||"+MemId+"\r\n";
            ...
            Query = "Select CASE WHEN Race = 1 THEN 'Black or African American' WHEN Race = 2 THEN 'American Indian' " +
                    "WHEN Race = 3 THEN 'Asian' WHEN Race = 4 THEN 'Native Hawaiian or Other Specific Islander' " +
                    "WHEN Race = 5 THEN 'White' WHEN Race = 6 THEN 'Others' ELSE 'Others' END " +
