...
//            final boolean useTls = false;
            final String Host = "192.168.110.10";
            clientid = "9";
            clientdb = "victoria";
...
//                        }
                        ...
                        switch (RequestType) {
                            case 1: {
                                ...
                                //hstmt2.executeUpdate("UPDATE request SET status=1,posttime=now(), MSCID = '" + MSCID + "' WHERE id=" + Id);
...
/*        int SelfPayChk = 0;
        ...
        try {
            ...
            Query = " Select Title, FirstName, MiddleInitial, LastName, MaritalStatus, MRN, DOB, Age, Gender, Email, " +
                    "PhNumber, Address, City, State, Country, ZipCode, SSN, Occupation, Employer, EmpContact, " +
                    "PriCarePhy, ReasonVisit, SelfPayChk, CreatedDate,IFNULL(sync,0) ,ID " +
                    " from victoria.PatientReg where MRN ='" + mrn.trim() + "' AND Status = 0";
...
//                System.out.println("HealthInsuranceChk Inside: " + HealthInsuranceChk);
                Query = " Select IFNULL(HIPrimaryInsurance,''), IFNULL(HISubscriberGroupNo,''), " +
                        "IFNULL(HISubscriberPolicyNo,'') " +
...
//                IN1 = "IN1|1|||" + HIPriInsurance + "|^^^^||||||||||G|||"+HISubscriberGroupNo+"||Y||||||||||||||||"+HISubscriberPolicyNo+"|||||||F||||M\r\n";
                IN1 = "IN1|1|||" + HIPriInsurance + "|^^^^|||" + HISubscriberGroupNo + "|||||||G|||||Y||||||||||||||||" + HISubscriberPolicyNo + "|||||||F||||M\r\n";
            }
...
//                    + "PV1||3^E/R^02|||||" + Address + "^^" + City + "^" + State + "^" + ZipCode + "|||||||||||||||||||||||||||||||||||||" + MSH7 + "|" + MSH7 + "\r\n"
                    ...
                    + IN1;
            //+ NK1;
            String MSH = "MSH|^~\\&||294|ADT|3344|" + MSH7 + "||ADT^" + Synctype + "|" + MSH7 + "|P|2.3";
...
/*            Query = "update oe.request set msg = '" + msg + "' where id = '" + TraceId + "'";
            ...
            pushtologs("Exception (Victoria) : " + ee.getMessage());
            pushtologs("Exception (Victoria) : " + str);
            try {
/*                hstmt2 = conn.createStatement();
...
/*                hstmt2 = conn.createStatement();
                hstmt2.executeUpdate("update victoria.PatientReg set sync=1 where sync=0 and id=" + TraceId);
                hstmt2.close();*/
                pStmt = conn.prepareStatement("UPDATE " + clientdb + ".PatientReg SET sync= ? WHERE sync= ? AND id = ? ");
                pStmt.setInt(1, 1);
                pStmt.setInt(2, 0);
                pStmt.setString(3, TraceId);
                pStmt.executeUpdate();
