...
public class Orange_hl7loopsender implements Runnable {
    ...
    static {
        Orange_hl7loopsender.SignonDone = false;
    }

    private Socket sock;
    private boolean done;

    public Orange_hl7loopsender(final Socket socket) {
        this.done = false;
        this.sock = socket;
        TraceNo = 0;
    }
...
//                        }
                        ...
                        Thread.sleep(750L);
                        final Orange_hl7loopsender client = new Orange_hl7loopsender(sock);
...
/*        String AddressIfDifferent = "";
        ...
        try {
            ...
            Query = "Select Title, FirstName, MiddleInitial, LastName, IFNULL(MaritalStatus,''), MRN, DOB, Age, Gender, " +
                    ...
                    "IFNULL(Race,''), CASE WHEN Ethnicity = 1 THEN 'Hispanic' WHEN Ethnicity = 2 THEN 'Non-Hispanic' " +
...
//            String GT1="GT1|1|"+"230"+"||||"+""+""+""+"^^CP^^^|||";
            ...
            //GT1|1|230|VALANCE^LIBERTY^L||1 SHINBONE WAY^^TEXARKANA^TX^75501|(904)555-1842^^CP^^^904^5551842||19840701|F
...
//            sock.getOutputStream().write(byteBuffer);
            ...
            pushtologs("Exception (Orange- GTEC) : " + ee.getMessage());
            pushtologs("Exception (Orange- GTEC) : " + str);
