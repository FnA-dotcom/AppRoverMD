...
public class Schertz_hl7Looper implements Runnable {
    ...
    static {
        Schertz_hl7Looper.SignonDone = false;
    }

    private Socket sock;
    private boolean done;

    public Schertz_hl7Looper(final Socket socket) {
        this.done = false;
        this.sock = socket;
        TraceNo = 0;
    }
...
//                    hstmt2 = conn.createStatement();
                    ...
                    while (rset.next()) {
                        ...
                        Thread.sleep(750L);
                        final Schertz_hl7Looper client = new Schertz_hl7Looper(sock);
    ...
    public static void sendmsg2(final Socket sock, String msg, final String mrn, final int flag, final Connection conn, String MSCID) {
        ...
        try {
            ...
            String MSH7 = MSCID;
            msg = "MSH|^~\\&||415|ADT|3344|" + MSH7 + "||ADT^" + Synctype + "|" + MSH7 + "|P|2.3\r\n"
                    + "EVN|" + Synctype + "|" + MSH7 + "|||XXX^^^^^^^^488 \r\n"
                    + "PID|1||" + MRN + "||" + LastName + "^" + FirstName + "^" + MiddleInitial + "||" + DOB + "|" + gender + "||" + Race + "|" + Address + "^^" + City + "^" + State + "^" + ZipCode + "|" + "" + "|" + PhNumber + "^^CP^^^" + "" + "^" + "" + "|" + EmpContact + "^^PH^" + Email + "^^" + "" + "||" + MaritalStatus + "||" + VisitNumber + "|" + SSN + "|||" + Ethnicity + "\r\n"
                    + "PV1||3^E/R^02|EMERGENCY ROOM|||||||||||||||||||||||||||||||||||||||||" + MSH7 + "|\r\n"
                    + "GT1|1|" + "230" + "|" + "" + "^" + GuarantorName + "^" + "||" + "" + "|" + GuarantorNumber + "^^CP^^^" + "" + "||" + "" + "|" + "" + "\r\n"
                    + IN1
                    + NK1;

            String MSH = "MSH|^~\\&||415|ADT|3344|" + MSH7 + "||ADT^" + Synctype + "|" + MSH7 + "|P|2.3";
            ...
            pushtologs("Exception (Schertz) : " + ee.getMessage());
            pushtologs("Exception (Schertz) : " + str);
