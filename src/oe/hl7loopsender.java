...
public class hl7loopsender implements Runnable {
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            System.out.println("Connecting To  Epower Doc socket  @  192.168.230.6 8097");
            sock = new Socket("192.168.230.6", 8097);
            System.out.println("|||||||||||");
        } catch (Exception e) {
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    Query = " SELECT id,msg,mrn,flag FROM request WHERE status=0 and ClientIndex = 8 ORDER BY requestdate";
                    ...
                    while (hrset.next()) {
                        ...
                        Thread.sleep(750L);
                        final hl7loopsender client = new hl7loopsender(sock);
                ...
                if ((dt2.getTime() - StartTime) / 1000L > 45.0) {
                    final hl7loopsender client2 = new hl7loopsender(sock);
    ...
    public static void sendmsg2(final Socket sock, String msg, final String mrn, final String flag, final Connection conn) {
        ...
        try {
            ...
            Query = "Select Title, FirstName, MiddleInitial, LastName, MaritalStatus, MRN, DOB, Age, Gender, Email, PhNumber, Address, City, State, Country,  ZipCode, SSN, Occupation, Employer, EmpContact, PriCarePhy, ReasonVisit, SelfPayChk, CreatedDate,sync,ID, IFNULL(Race,''), " +
                    "CASE WHEN Ethnicity = 1 THEN 'Hispanic' WHEN Ethnicity = 2 THEN 'Non-Hispanic' WHEN Ethnicity = 3 THEN 'Others' ELSE 'Others' END from oe_2.PatientReg where MRN ='" + mrn.trim() + "'";
            ...
            System.out.println(MaritalStatus + "--:After");
            ...
            if (SelfPayChk == 1) {
                Query = " Select IFNULL(WorkersCompPolicy,0), IFNULL(MotorVehAccident,0), IFNULL(PriInsurance,'-'),IFNULL(MemId,'-'), " +
                        ...
                        "FROM oe_2.InsuranceInfo  where PatientRegId = " + Id;
                System.out.println(Query);
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                while (rset.next()) {
...
//                IN1 = "IN1|1|||" + PriInsurance + "|^^^^|||" + GrpNumber + "||||||||||||||||||||||||||||"+MemId+"|||||||||||||"+MemId+"\r\n";
            ...
            Query = "Select IFNULL(NextofKinName,''), IFNULL(RelationToPatient,''), IFNULL(PhoneNumber,'') " +
                    "from oe_2.EmergencyInfo where PatientRegId = " + Id;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            Query = "Select CASE WHEN Race = 1 THEN 'Black or African American' WHEN Race = 2 THEN 'American Indian' WHEN Race = 3 THEN 'Asian' WHEN Race = 4 THEN 'Native Hawaiian or Other Specific Islander' WHEN Race = 5 THEN 'White' WHEN Race = 6 THEN 'Others' ELSE 'Others' END " +
                    "from oe_2.PatientReg_Details where PatientRegId = " + Id;
            //System.out.println(Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
...
//                    + "PID|1||" + MRN + "||"+LastName + "^" + FirstName + "^" + MiddleInitial + "||" + DOB + "|" + gender + "||"+Race+"| " + Address + "^^" + City + "^" + State + "^" + ZipCode + "|" + "" +"|" + PhNumber + "|||"+MaritalStatus+"|||" + SSN +"|||11\r\n"
