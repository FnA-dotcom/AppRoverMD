...
public class Frontline_hl7loopsender implements Runnable
{
    ...
    static {
        Frontline_hl7loopsender.SignonDone = false;
    }

    public Frontline_hl7loopsender(final Socket socket) {
        this.done = false;
        this.sock = socket;
        Frontline_hl7loopsender.TraceNo = 0;
    }
    ...
    public static void main(final String[] args) {
        ...
        try {
            ...
            final String Host = "10.10.20.45";
            System.out.println("Connecting To  Database  @  localhost ");
            conn = getConnection();
            System.out.println("Connection: " + conn);
            System.out.println("Connecting To  Epower Doc socket  @  10.10.20.45 10010");
            sock = new Socket("10.10.20.45", 10010);
            System.out.println("|||||||||||");
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    //Query = " SELECT id,msg,mrn,flag FROM request WHERE status=0 and ClientIndex = 27 ORDER BY requestdate";
                    Query = " SELECT id,msg,mrn,flag FROM request WHERE status=0 and ClientIndex = 27 LIMIT 2";
                    ...
                    while (hrset.next()) {
                        ...
                        Thread.sleep(750L);
                        final Frontline_hl7loopsender client = new Frontline_hl7loopsender(sock);
                ...
                if ((dt2.getTime() - StartTime) / 1000L > 45.0) {
                    final Frontline_hl7loopsender client2 = new Frontline_hl7loopsender(sock);
    ...
    public static void sendmsg2(final Socket sock, String msg, final String mrn, final String flag, final Connection conn) {
        ...
        try {
            ...
            Query = "Select Title, FirstName, MiddleInitial, LastName, MaritalStatus, MRN, DOB, Age, Gender, Email, PhNumber, Address, City, State, Country,  ZipCode, SSN, Occupation, Employer, EmpContact, PriCarePhy, ReasonVisit, SelfPayChk, CreatedDate,sync,ID, " +
                    "CASE WHEN Ethnicity = 1 THEN 'Hispanic' WHEN Ethnicity = 2 THEN 'Non-Hispanic' WHEN Ethnicity = 3 THEN 'Others' ELSE 'Others' END from frontlin_er.PatientReg where MRN ='" + mrn.trim() + "'";
            ...
            if (SelfPayChk == 1) {
                ...
                if (!PriInsuranceName.equals("-")) {
                    Query = " Select PayerName from frontlin_er.ProfessionalPayers where Id =  " + PriInsuranceName;
                    //System.out.println(Query);
                    stmt = conn.createStatement();
                    rset = stmt.executeQuery(Query);
                    while (rset.next()) {
...
//                IN1 = "IN1|1|" + MemId + "||" + PriInsurance + "||||" + GrpNumber + "|||||||||||||||||||||||||||||||||||";
            ...
            Query = "Select IFNULL(NextofKinName,''), IFNULL(RelationToPatient,''), IFNULL(PhoneNumber,'') from frontlin_er.EmergencyInfo where PatientRegId = "+Id;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            Query = "Select CASE WHEN Race = 1 THEN 'Black or African American' WHEN Race = 2 THEN 'American Indian' WHEN Race = 3 THEN 'Asian' WHEN Race = 4 THEN 'Native Hawaiian or Other Specific Islander' WHEN Race = 5 THEN 'White' WHEN Race = 6 THEN 'Others' ELSE 'Others' END " +
                    "from frontlin_er.PatientReg_Details where PatientRegId = "+Id;
            //System.out.println(Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
...
//                    + IN1;
            ...
            msg = "MSH|^~\\&||294|ADT|3344|" + MSH7 + "||ADT^" + Synctype + "|" + MSH7 + "|P|2.3\r\n"
                    + "EVN|" + Synctype + "|20200707020302|||XXX^^^^^^^^488 \r\n"
                    //+ "PID|1||" + MRN + "||" + LastName + "^" + FirstName + "^" + MiddleInitial + "||" + DOB + "|" + gender + "||W|" + Address + "^^" + City + "^" + State + "^" + ZipCode + "|" + PhNumber + "|" + PhNumber + "||ENGLISH|M|001||" + SSN + "|||||||||||N \r\n"
...
//                    +"GT1|1|"+"230"+"|"+"Test^Test^M"+"||"+""+"|"+"(222)555-5698"+"^^CP^^^"+""+"||"+""+"|"+""+"\r\n"
            ...
            hstmt2 = conn.createStatement();
            hstmt2.executeUpdate("update frontlin_er.PatientReg set sync=1 where sync=0 and id=" + Id);
