...
public class RegisteredPatients11FEB2021 extends HttpServlet {
    static String DOS = "";
    static String Acct = "";
        ...
        try {
            ...
            if (PatientCount > 0) {
                ...
                Query = "SELECT CONCAT(IFNULL(Title,''),' ',IFNULL(FirstName,''),' ',IFNULL(MiddleInitial,''),' ',IFNULL(LastName,'')), DATE_FORMAT(DOB,'%m/%d/%Y'), IFNULL(PhNumber,''), " +
                        ...
                        "CASE WHEN COVIDStatus = 1 THEN 'POSITIVE' WHEN COVIDStatus = 0 THEN 'NEGATIVE'  WHEN COVIDStatus = -1 THEN 'SUSPECTED' ELSE 'UN-EXAMINED' END " +
    ...
    void SaveInvoice(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, final HttpServletResponse response, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            if (TotalAmount == 0.00) {
                ...
                Parser.SetField("Message", "Amount cannot be zero. Please select atleast one test!");
            ...
            try {
                ...
                out.println("Error 1-  in Getting Invoice No: " + e.getMessage());
            }

            try {
                PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".InvoiceMaster (PatientMRN,InvoiceNo,TotalAmount ,PaidAmount," +
                                "Paid,PaymentDateTime,InvoiceCreatedBy ,CreatedDate, Status, BalAmount,InstallmentApplied)" +
                                "  VALUES (?,?,?,?,?,?,?,now(),0,?,0) ");
            ...
            try {
                ...
                out.println("Error 3- in Getting InvoiceMasterID: " + e.getMessage());
            }
            try {
                for (i = 0; i < RowCount; ++i) {
/*                    Query = "Select Cost from " + Database + ".SelfPaySheet where Id = " + Integer.parseInt(InvoiceTableInput[i][4]);
                ...
                out.println("Error 4- Insertion InvoiceDetail Table :" + e.getMessage());
            }
            out.println("1");
        } catch (Exception var11) {
    ...
    private void InvoicePdf(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final Connection conn, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            if (Database.equals("oe_2")) {
                ...
                jpeg = Image.getInstance("/sftpdrive/opt/apache-tomcat-7.0.65/webapps/md/images_/jpg/" + ClientId + ".jpg");
            }
            if (jpeg != null) {
                jpeg.setAlignment(3);
    ...
    void CollectPayment(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            Query = " SELECT a.PatientMRN, CONCAT(b.Title,' ',b.FirstName,' ',b.MiddleInitial,' ',b.LastName), " +
                    " DATE_FORMAT(b.DOB,'%m/%d/%Y') FROM " + Database + ".InvoiceMaster a " +
...
//        CDRList.append("<td align=left><a href=" + DocumentPath + " target=\"_blank\">" + "ViewInvoice_" + rset.getString(1) + "</a></td>\n");
    ...
    void PayNow(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            Query = "Select a.PatientMRN, CONCAT(b.Title,' ',b.FirstName,' ',b.MiddleInitial,' ',b.LastName), " +
                    "a.TotalAmount, a.PaidAmount, a.BalAmount  from " + Database + ".InvoiceMaster a " +
                    " LEFT JOIN " + Database + ".PatientReg b on a.PatientMRN = b.MRN " +
