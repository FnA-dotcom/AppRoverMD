...
//                return;
    ...
    void EditValues(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, final HttpServletResponse response, UtilityHelper helper) {
        try {
            Statement stmt = null;
            ResultSet rset = null;
            String Query = "";
            ...
            try {
                Query = " Select IFNULL(Title,''), IFNULL(FirstName,''), IFNULL(LastName,''), IFNULL(MiddleInitial, ''), " +
                        ...
                        "IFNULL(County,''), ID from " + Database + ".PatientReg where MRN = '" + MRN + "'";
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                if (rset.next()) {
            ...
            try {
                Query = " Select IFNULL(Ethnicity,''), IFNULL(EmployementChk,'0'), IFNULL(Employer,''), IFNULL(Occupation, ''), IFNULL(EmpContact,''),  " +
                        ...
                        " IFNULL(SympOthersTxt,'') , IFNULL(EmpHealthChk,'0'),IFNULL(PregChk,'0') " +
            ...
            if (SympChkCOVID.equals("0")) {
                SympChkCOVIDBuffer.append("<div class=\"controls\">\n" +
                        ...
                        "</fieldset>\n" +
                        "<fieldset>\n" +
                        "<input name=\"SympChkCOVID\" type=\"radio\" id=\"SympChkCOVID_No\" value=\"0\" checked>\n" +
                ...
                SympChkCOVIDBuffer.append("<div class=\"controls\">\n" +
                        "<fieldset>\n" +
                        "<input name=\"SympChkCOVID\" type=\"radio\" id=\"SympChkCOVID_Yes\" value=\"1\" checked>\n" +
                        "<label for=\"SympChkCOVID_Yes\">Yes</label>\n" +
                        "</fieldset>\n" +
                        "<fieldset>\n" +
...
/*                Query = " Update " + Database + ".PatientReg_Details Set Ethnicity = '" + Ethnicity + "', Race = '" + Race + "', " +
                ...
                pStmt = conn.prepareStatement(" Update " + Database + ".PatientReg_Details Set Ethnicity = ?, Race = ?, " +
                        ...
                        " SympOthers = ?, SympOthersTxt = ?, EmpHealthChk = ?, PregChk = ? " +
                        " where PatientRegId = ? ");
