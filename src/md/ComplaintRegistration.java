...
//            out.println("inside  Dashboard try ");
            Query = "SELECT Activity,Sub_Activity,Priority,Brief_Description,Details,`Status`,Date_created,Activity_ID FROM frontlin_er.Complaints ORDER BY Date_created DESC";
...
//            out.println("inside  Dashboard query executed.... ");
            ...
            try {
                Query = "SELECT Assign_to_Member,COUNT(*) FROM frontlin_er.ComplaintsAssignment GROUP BY Assign_to_Member ORDER BY Assign_to_Member DESC LIMIT 4";
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                int i = 0;
                while (rset.next()) {
...
//            out.println("inside  Dashboard Generating Html.... ");
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/Dashboard.html");
//            out.println("inside  Dashboard Generated Html.... ");
    ...
    private void RegisterComplaint(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws IOException {
        ...
        int num = 0;
        try {
            Query = "Select Activity_ID from frontlin_er.Complaints ORDER BY Date_created DESC LIMIT 1";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
        ...
        Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/RegisterComplaint.html");
    }
...
//            out.println("Before Query: "+Query);
            Query = "Select Sub_Activities from oe.ComplaintsRegistrationActivities where Activities = '" + request.getParameter("operation") + "'";
...
//        out.println("getSubActivities: ");
        ...
        try {
//            out.println("Before Query: "+Query);
            Query = "Select DISTINCT(Team) from oe.ComplaintsRegistrationTeams";
//            out.println("Query: "+Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
//        out.println("query executed");
...
//        out.println("getSubActivities: ");
        ...
        try {
//            out.println("Before Query: "+Query);
            Query = "Select Members from oe.ComplaintsRegistrationTeams WHERE Team='" + request.getParameter("operation") + "'";
//            out.println("Query: "+Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
//        out.println("query executed");
    ...
    private void insertComplaint(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) {
        ...
        try {
            ...
            PreparedStatement MainReceipt = conn.prepareStatement(
                    "INSERT INTO frontlin_er.Complaints (Activity,Sub_Activity,Priority,Brief_Description,Details,Status,Date_created,Activity_ID) VALUE(?,?,?,?,?,?,now(),?)");
            ...
            if (Action.equals("Assign")) {
                ...
                MainReceipt = conn.prepareStatement("INSERT INTO frontlin_er.ComplaintsAssignment (Activity_ID,Assign_TO,Assign_to_Member,Assigned_Date) VALUE(?,?,?,now())");
...
//            out.println("inside  FollowUp try ");
            Query = "SELECT Complaints.Activity, Complaints.Sub_Activity,Complaints.Priority,Complaints.Brief_Description,Complaints.Details,Complaints.`Status`,Complaints.Date_created,Complaints.Activity_ID," +
                    ...
                    " FROM frontlin_er.Complaints " +
                    "INNER JOIN frontlin_er.ComplaintsAssignment" +
...
//            out.println("inside  FollowUp query executed.... ");
            while (rset.next()) {
                if (!(list.contains(rset.getString(8)))) {
                    ...
                    CDRList.append("<td align=left><a class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" href=\"/md/md.ComplaintRegistration?ActionID=GetDetailsFollowUp&ID=" + act[0] + "&act=" + act[1] + "\">Detail</button></td>\n");
...
//            out.println("inside  FollowUp Data Fetched.... ");
            ...
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("CDRList", String.valueOf(CDRList));
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/FollowUpComplaint.html");
    ...
    private void Assign(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws ServletException {
        ...
        try {
//            out.println("inside  Dashboard try ");
            Query = "SELECT Activity,Sub_Activity,Priority,Brief_Description,Details,`Status`,Date_created,Activity_ID FROM frontlin_er.Complaints WHERE `Status` = \"Hold/Unassigned\" ORDER BY Date_created DESC";
...
//            out.println("inside  Dashboard query executed.... ");
            while (rset.next()) {
                ...
                CDRList.append("<td align=left><a class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" href=\"/md/md.ComplaintRegistration?ActionID=GetDetails&ID=" + act[0] + "&act=" + act[1] + "\">Detail</button></td>\n");
...
//            out.println("inside  Dashboard Data Fetched.... ");
            ...
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("CDRList", String.valueOf(CDRList));
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/AssignComplaint.html");
    ...
    private void GetDetails(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws ServletException {
        ...
        try {
            Query = "Select Activity_ID,Activity,Sub_Activity,Priority,Brief_Description,Details,`Status`  from frontlin_er.Complaints WHERE Activity_ID='" + ID + "#" + act + "'";
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/ComplaintDetails.html");
        } catch (Exception e) {
            e.getStackTrace();
        }
...
//            out.println("inside GetDetailsFollowUp Query executed...");
            ...
            try {
                Query = "SELECT Assign_TO,Assigned_Date,IFNULL(Assign_to_Member,'Whole Team'),IFNULL(Remarks,'No Remarks') FROM frontlin_er.ComplaintsAssignment where Activity_ID='" + ID + "#" + act + "' ORDER BY Assigned_Date DESC;";
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/ComplaintDetailsFollowUp.html");
        } catch (Exception e) {
            e.getStackTrace();
        }
    ...
    private void SaveComplaint(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws ServletException {
        ...
        try {
            ...
            PreparedStatement MainReceipt = conn.prepareStatement(
                    "INSERT INTO frontlin_er.ComplaintsAssignment (Activity_ID,Assign_TO,Assign_to_Member,Remarks,Assigned_Date) VALUE(?,?,?,?,now())");
            ...
            Query = "UPDATE frontlin_er.Complaints SET Status = '" + Action + "' WHERE Activity_ID = '" + Activity_ID + "'";
...
//            out.println("inside  Closure try ");
            Query = "SELECT Complaints.Activity, Complaints.Sub_Activity,Complaints.Priority,Complaints.Brief_Description,Complaints.Details,Complaints.`Status`,Complaints.Date_created,Complaints.Activity_ID,\n" +
                    "ComplaintsAssignment.Assign_TO,ComplaintsAssignment.Assigned_Date\n" +
                    "FROM frontlin_er.Complaints\n" +
                    "Left JOIN frontlin_er.ComplaintsAssignment\n" +
                    " ON Complaints.Activity_ID=ComplaintsAssignment.Activity_ID\n" +
...
//            out.println("inside  Closure query executed.... ");
            while (rset.next()) {
                if (!(list.contains(rset.getString(8)))) {
                    ...
                    CDRList.append("<td align=left><a class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" href=\"/md/md.ComplaintRegistration?ActionID=GetDetailsClosure&ID=" + act[0] + "&act=" + act[1] + "\">Detail</button></td>\n");
...
//            out.println("inside  Closure Data Fetched.... ");
            ...
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("CDRList", String.valueOf(CDRList));
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/ClosureComplaint.html");
...
//                out.println("Remarks: "+Remarks);
            ...
            Parser.SetField("Assign_TO", Assign_TO);
            Parser.SetField("Remarks", Remarks);
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/ComplaintDetailsClosure.html");
        } catch (Exception e) {
            e.getStackTrace();
        }
...
//            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Exception/Message.html");
    ...
    private void TeamMember(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws ServletException {
        ...
        try {
            Query = "SELECT DISTINCT(Team) FROM `ComplaintsRegistrationTeams`";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
...
//                CDRList.append(rset.getString(1));
                Query = "SELECT IFNULL(Members,'Have No Member') FROM `ComplaintsRegistrationTeams` WHERE Team='" + rset.getString(1) + "'";
            ...
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("CDRList", String.valueOf(CDRList));
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Complaint/AddTeamMember.html");
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void AddTeam(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws IOException {
        ...
        try {
            PreparedStatement MainReceipt = conn.prepareStatement("INSERT INTO `ComplaintsRegistrationTeams` (Team) VALUE(?)");
    ...
    private void AddMember(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, int facilityIndex, UtilityHelper helper) throws IOException {
        ...
        try {
            ...
            Query = "SELECT Count(*) from ComplaintsRegistrationTeams where Team = \'" + Add_Team + "\' and Members Is NULL";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            ...
            if (null_data_found > 0) {
                PreparedStatement MainReceipt = conn.prepareStatement("UPDATE ComplaintsRegistrationTeams SET Members = \'" + Add_Member + "\'  WHERE Team = \'" + Add_Team + "\' and Members is NULL");
                MainReceipt.executeUpdate();
                MainReceipt.close();
            } else {
                PreparedStatement MainReceipt = conn.prepareStatement("INSERT INTO `ComplaintsRegistrationTeams` (Team,Members) VALUE(?,?)");
                MainReceipt.setString(1, Add_Team);
