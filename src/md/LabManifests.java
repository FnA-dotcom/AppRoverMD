        ...
        try {
            ...
            switch (ActionID) {
                ...
                case "manifestPDF":
                    ...
                    break;
                case "manifestPDF_test":
                    supp.Dologing(UserId, conn, request.getRemoteAddr(), ActionID, "Saving Manifests of Lab", "Saving.... Manifest", FacilityIndex);
                    manifestPDF_test(request, out, conn, context, response, UserId, DatabaseName, FacilityIndex, DirectoryName, 0);
                    break;
                default: {
    ...
    private void GetInput(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, HttpServletResponse response, String UserId, String Database, int ClientId, UtilityHelper helper, String locationArray) throws FileNotFoundException {
        ...
        try {
            ...
            Query = " SELECT CONCAT(IFNULL(a.FirstName,''),' ',IFNULL(a.MiddleInitial,''),' ',IFNULL(a.LastName,'')), " + //1
                    ...
                    " ELSE 'Pending' END,d.Location, " + //11
                    " b.ID AS orderid, IFNULL(e.sampleNumber,0) " +//13
                    ...
                    " INNER JOIN " + Database + ".Tests e ON e.orderid = b.Id   " +
                    " WHERE a.Status = 0 AND b.StageIdx = 0 AND e.TestIdx !=4 " + Defaultlocation +
            ...
            while (rset.next()) {
                ...
                String sampleNumflag = "";
                String sampleNum = rset.getString(13);
                System.out.println("sampleNum " + sampleNum);
                if (sampleNum.length() == 0) {
                    sampleNumflag = "value=\"_M_\"";
                    sampleNum = "<font color=\"red\">Missing</font>";
                }

                ManifestList.append("<td align=left><input  class='checkSingle' type='checkbox'  " + sampleNumflag + "  name='mList_" + rset.getInt(12) + "'  id='mList_" + rset.getInt(12) + "' > \n");

                ManifestList.append("<label for=\"mList_" + rset.getInt(12) + "\">" + rset.getString(8) + "</label><br></td>\n");//OrderID  class"form-check-input"
                ManifestList.append("<td align=left>" + rset.getString(4) + "</td>\n");//MRN
                ManifestList.append("<td align=left>" + rset.getString(1) + "</td>\n");//PatientName
                ManifestList.append("<td align=left>" + sampleNum + "</td>\n"); //specimin
    ...
    void SaveManifest(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, HttpServletResponse response, final String UserId, final String Database, final int ClientId, UtilityHelper helper, String directoryName) throws ServletException, IOException {
        ...
        for (String key : paramMap.keySet()) {
            ...
            if (key.startsWith("mList_")) {
                String value__ = request.getParameter(key);
                if (value__.compareTo("_M_") == 0) {


                    Parsehtm Parser = new Parsehtm(request);
                    Parser.SetField("Message", "Few Sample Number is Missing");
                    Parser.SetField("FormName", "LabManifests");
                    Parser.SetField("ActionID", "GetInput");
                    Parser.SetField("Defaultlocation", Defaultlocation);
                    Parser.SetField("Filter", Defaultlocation);
                    Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Exception/Manifest_missing_sample.html");
                    return;
        ...
        try {
            ...
            int seqNo = 0;
            try {
                Query = "Select IFNULL(MAX(Convert(Substring(ManifestNum,14,9),UNSIGNED INTEGER)),0) + 1 ," +
...
//                out.println("Error 3- :" + e.getMessage());
            ...
            while (keys.hasMoreElements()) {
                ...
                if (key.startsWith("mList_")) {
                    ...
                    try {
                        Query = "SELECT PatRegIdx FROM " + Database + ".TestOrder " +
                                "WHERE Id = " + key;
                        stmt = conn.createStatement();
                        rset = stmt.executeQuery(Query);
                        if (rset.next())
           ...
           /* for (int i = 0; i < selectedManifest.length; i++) {
            ...
            // Exception/MessageRoverLab_manifest.html
            ...
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("Message", "Thank You Press ok to Download Manifest");
            Parser.SetField("manifestIdx", String.valueOf(manifestIdx));

            Parser.GenerateHtml(out, String.valueOf(Services.GetHtmlPath(getServletContext())) + "Exception/MessageRoverLab_manifest.html");
...
//            System.out.println("outputFilePath " + outputFilePath);
            ...
            String samplecount = LabManifestsRetrieve.getsamplecount(conn, Database, String.valueOf(manifestid));
            ...
            while (rset.next()) {
                ...
                String BarCodeFilePath = barCode.GetBarCode(request, out, conn, servletContext, UserId, Database, ClientId, url);
                type1_02_c(pdfStamper, p, BarCodeFilePath, 270.0f, 720.0f);

                //QR Code
                //String QrodeFilePath = QRcodeV2.generateQRcode_v1(url, outputFilePath2, charset, 80, 120);
                //type1_02(pdfStamper, p, QrodeFilePath, 250.0f, 710.0f);
                type1_01(pdfStamper, p, url, 270.0F, 705.0F);
                type1_01(pdfStamper, p, "Location " + loc_liscode.get(rset.getString(4)), 30.0F, 710.0F);
                type1_01(pdfStamper, p, "Count : " + samplecount, 450.0F, 720.0F);
                type1_01(pdfStamper, p, "Created Date: " + rset.getString(5), 450.0F, 710.0F);
                type1_01(pdfStamper, p, "Created By: " + rset.getString(6), 450.0F, 700.0F);
                ...
                Query1 = "SELECT c.OrderNum, b.MRN," +
                        " CONCAT(IFNULL(b.FirstName,''),' ',IFNULL(b.MiddleInitial,''),' ',IFNULL(b.LastName,''))," +
                        "  e.sampleNumber,e.collectionDateTime,DATE_FORMAT(b.DOB,'%m/%d/%Y') " +
                        " FROM " + Database + ".ManifestDetails a " +
                        " INNER JOIN " + Database + ".PatientReg b ON a.PatRegIdx = b.ID " +
                        " INNER JOIN " + Database + ".TestOrder c ON a.OrderId = c.Id " +
                        " INNER JOIN " + Database + ".Tests e ON a.OrderId = e.OrderId " +
                        // " INNER JOIN "+Database+".ListofStages d ON c.StageIdx = d.Id " +
                        // " INNER JOIN "+Database+".Locations e ON b.TestingLocation = e.Id " +
                        " WHERE a.ManifestIdx = " + rset.getInt(1);
...
//                    System.out.println("Y-Axis --> " + yAxis);
                    type1_01(pdfStamper, p, rset1.getString(1), 20.0f, yAxis); //ORDER ID
//                    type1_01(pdfStamper, p, rset.getString(2), 100.0f, yAxis);//Manifest
                    type1_01(pdfStamper, p, rset1.getString(2), 100.0f, yAxis);//MRN
                    type1_01(pdfStamper, p, rset1.getString(3), 140.0f, yAxis);//PatName
                    type1_01(pdfStamper, p, rset1.getString(6), 320.0f, yAxis);//DOB
                    type1_01(pdfStamper, p, rset1.getString(4), 400.0f, yAxis);//Sample
                    type1_01(pdfStamper, p, rset1.getString(5), 480.0f, yAxis);//Collection time
                    yAxis = yAxis - 15;
                ...
                PreparedStatement MainReceipt = conn.prepareStatement(
                        " UPDATE " + Database + ".ManifestsMaster SET isGeneratedPDF = 0,isRetrieved=1 " +
                                "WHERE isRetrieved=0 and ManifestsMaster.Id = ?");
    ...
    void manifestPDF_2(HttpServletRequest request, PrintWriter out, Connection conn,
                       ServletContext servletContext, HttpServletResponse response, String UserId,
                       String Database, int ClientId, String DirectoryName, int manifestid) {
        Statement stmt = null;
        ResultSet rset = null;
    ...
    void manifestPDF_test(HttpServletRequest request, PrintWriter out, Connection conn,
                          ServletContext servletContext, HttpServletResponse response, String UserId,
                          String Database, int ClientId, String DirectoryName, int manifestid) {
        Statement stmt = null;
        ResultSet rset = null;
...
//   System.out.println("outputFilePath " + outputFilePath);
            ...
            while (rset.next()) {
                ...
                String QrodeFilePath = QRcodeV2.generateQRcode_v1(url, outputFilePath2, charset, 200, 180);
                type1_02_c(pdfStamper, p, BarCodeFilePath, 270.0f, 720.0f);
                type1_01(pdfStamper, p, url, 270.0F, 705.0F);
                type1_01(pdfStamper, p, "Location " + loc_liscode.get(rset.getString(4)), 30.0F, 710.0F);
                type1_01(pdfStamper, p, "Count : " + samplecount, 450.0F, 720.0F);
                ...
                Query1 = "SELECT c.OrderNum, b.MRN," +
                        ...
                        " INNER JOIN " + Database + ".PatientReg b ON a.PatRegIdx = b.ID " +
                        " INNER JOIN " + Database + ".TestOrder c ON a.OrderId = c.Id " +
                        " INNER JOIN " + Database + ".Tests e ON a.OrderId = e.Id ";
                // " INNER JOIN "+Database+".ListofStages d ON c.StageIdx = d.Id " +
                // " INNER JOIN "+Database+".Locations e ON b.TestingLocation = e.Id " +
                // " WHERE a.ManifestIdx = " + rset.getInt(1);
                System.out.println("QQQ" + Query1);
...
//           type1_01(pdfStamper, 1, rset.getString(2), 100.0f, yAxis);//Manifest
                 ...
                 * PreparedStatement MainReceipt = conn.prepareStatement(
                 * " UPDATE "+Database+".ManifestsMaster SET isGeneratedPDF = 0,isRetrieved=1 "
                 * + "WHERE ManifestsMaster.Id = ?"); MainReceipt.setInt(1, rset.getInt(1));
                 * MainReceipt.executeUpdate(); MainReceipt.close();
                 */
            }
            rset.close();
            stmt.close();
    ...
    public void type1_02_c(PdfStamper stamper, int pageno, String Data, Float x, Float y) throws DocumentException, IOException {
