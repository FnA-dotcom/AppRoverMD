...
public class PatientUpdateInfo_old extends HttpServlet {
    ...
    void GetInput(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            Query = "SELECT CONCAT(IFNULL(Title,''),' ',IFNULL(FirstName,''),' ',IFNULL(MiddleInitial,''),' ',IFNULL(LastName,'')), DATE_FORMAT(DOB,'%m/%d/%Y'), IFNULL(PhNumber,''), " +
                    ...
                    "CASE WHEN COVIDStatus = 1 THEN 'POSITIVE' WHEN COVIDStatus = 0 THEN 'NEGATIVE'  WHEN COVIDStatus = -1 THEN 'SUSPECTED' ELSE 'UN-EXAMINED' END, IFNULL(Address,''), " +
            ...
            if (COVIDStatus.toUpperCase().equals("POSITIVE")) {
                ...
                CovidBuffer.append("<select class=\"form-control \" id=\"PayMethod\" name=\"PayMethod\" onchange=\"SaveCovidStatus(this.value)\">\n" +
                        ...
                        "<option value=\"1\" > Positive</option>\n" +
                        "<option value=\"0\" > Negative</option>\n" +
                        "<option value=\"-1\" selected> Suspected</option>\n" +
                        "</select>\n");
            } else {
                CovidBuffer.append("<select class=\"form-control \" id=\"PayMethod\" name=\"PayMethod\" onchange=\"SaveCovidStatus(this.value)\">\n" +
                        "<option value=\"\"> Please Select Any</option>\n" +
            ...
            Query = " Select IFNULL(MRN,''), IFNULL(Notes,''), IFNULL(DATE_FORMAT(CreatedDate,'%m/%d/%Y %T'),''), IFNULL(CreatedBy,'') " +
                    " from " + Database + ".Notes where PatientRegId = " + ID + " and MRN = " + MRN + " order by Id desc ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
//                out.println(Query);
            while (rset.next()) {
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Forms/PatientUpdateInfo_old.html");
    ...
    void AddNotes(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId) {
        ...
        try {
            ...
            Services.DumException("PatientUpdateInfo_old", "AddNotes 2- Insertion Notes Table :", request, e, this.getServletContext());
            return;
        }
    ...
    void UpdateDoctorId(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId) {
        ...
        try {
            ...
            Services.DumException("PatientUpdateInfo_old", "AddNotes 2- Updating Patient RegT able :", request, e, this.getServletContext());
            return;
        }
...
//                System.out.println(Query);
            ...
            Services.DumException("PatientUpdateInfo_old", "AddNotes 2- Inserting PAtientr Addistional Info :", request, e, this.getServletContext());
            return;
        }
