...
public class CptRevenueForMapping extends HttpServlet {
    ...
    public void handleRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            try {
                ...
                e.printStackTrace();
                helper.SendEmailWithAttachment("Error in CptRevenueForMapping ** (handleRequest -- SqlException)", context, e, "CptRevenueForMapping", "handleRequest", conn);
                Services.DumException("CptRevenueForMapping", "Handle Request", request, e, getServletContext());
                Parsehtm Parser = new Parsehtm(request);
    ...
    void GetInput(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        int SNo = 1;
        try {
            Query = "Select Id, ifnull(CPTCode,''),ifnull(RevCode,'') from " + Database + ".CPT_Rev_Mapping  where Status = 0 order by CreatedDate DESC ";
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Forms/CPTRevenueForMapping.html");
...
//    String DBName = request.getParameter("FacilityName").trim();
        ...
        try {
            ...
            PreparedStatement MainReceipt = conn.prepareStatement(
                    "INSERT INTO " + Database + ".CPT_Rev_Mapping (CPTCode,RevCode,CreatedBy,Status,CreatedDate) VALUES (?,?,?,0,now())");
            MainReceipt.setString(1, CPT);
            MainReceipt.setString(2, RevCode);
            ...
            helper.SendEmailWithAttachment("Error in CptRevenueForMapping ** (ShowReport)", servletContext, e, "CptRevenueForMapping", "ShowReport", conn);
            Services.DumException("ShowReport", "CptRevenueForMapping ", request, e);
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("FormName", "ManagementDashboard");
...
//            System.out.println("FacIdx is----->"+FacIdx);
            ...
            for (i = 1; i <= totalrows; i++) {
                ...
                Matcher RevCodeMatcher = p.matcher(RevCode);
                boolean RevCodeSpecialCharacterFound = RevCodeMatcher.find();
                if (RevCodeSpecialCharacterFound == false) {
                    System.out.println("RevCodeSpecialCharacterFound" + RevCodeMatcher);
                    PatternFlag = 1;
                }
                SMSList.append("<tr>");
...
//            Parser.SetField("Facility", String.valueOf(Facility));
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Forms/sendCPTRevenueForMappingFile.html");
...
//        String CPT = request.getParameter("CPT").trim();
        ...
        try {
            for (int i = 1; i <= totalrows; i++) {
                ...
                PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".CPT_Rev_Mapping (FileName,CreatedBy,CPTCode,RevCode,Status,CreatedDate) VALUES (?,?,?,?,0,now())");
                MainReceipt.setString(1, FileName);

                MainReceipt.setString(2, UserId);

                MainReceipt.setString(3, CPT);
                MainReceipt.setString(4, RevCode);
                MainReceipt.executeUpdate();
                MainReceipt.close();
                System.out.println("hee is the querry" + MainReceipt);
            }
    ...
    void UpdateData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        String RevCode = request.getParameter("RevCode").trim();
        System.out.println("CPT--->" + RevCode);
        ...
        try {
            ...
            PreparedStatement MainReceipt = conn.prepareStatement(
                    "Update " + Database + ".CPT_Rev_Mapping set CPTCode =?,RevCode =?,ModifiedBy=?,Status=?,ModifiedDate =now()  where Id =" + Id);
            MainReceipt.setString(1, CPT);
            MainReceipt.setString(2, RevCode);
            MainReceipt.setString(3, UserId);
    ...
    void EditData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        try {
            Query = "Select  CPTCode,RevCode,Status From " + Database + ".CPT_Rev_Mapping where Id = " + Id;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
                CPT = rset.getString(1);
                RevCode = rset.getString(2);
            ...
            rset.close();
            stmt.close();
            out.println(CPT + "|" + RevCode + "|" + String.valueOf(Status) + "|" + String.valueOf(Id));
    ...
    void FacilityData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        int SNo = 1;
        try {
            Query = "Select a.Id, ifnull(a.CPTCode,''),ifnull(a.RevCode,''),ifnull(b.name,'') from " + DBName + ".CptRevenueForMapping a left join oe.clients b on a.FacilityName = b.dbname  where a.Status = 0 order by a.CreatedDate DESC ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
