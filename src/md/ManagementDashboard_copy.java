...
//        StringBuffer DoctorsList = new StringBuffer();
        ...
        try {
            ...
            Query = "Select COUNT(*) from " + Database + ".PatientVisit a " +
                    ...
                    "where " +
                    "DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') >= '" + TODAY + " 00:00:00' and " +
                    "DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') <= '" + TODAY + " 23:59:59' AND " +
                    "b.status=0";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            if (ClientId == 9 || ClientId == 28) {
                ...
                Query = "Select COUNT(*) from " + Database + ".PatientReg LEFT JOIN " + Database + ".PatientVisit ON PatientReg.ID=PatientVisit.PatientRegId \n" +
                        "where PatientReg.Status = 0 and " +
                        "PatientReg.SelfPayChk = 1 and " +
                        "PatientVisit.DateOfService >= '" + TODAY + " 00:00:00' and " +
                        "PatientVisit.DateOfService <= '" + TODAY + " 23:59:59'";
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                if (rset.next()) {
                    PatientCountInsured_TODAY = rset.getInt(1);
...
//            Query = "Select COUNT(*) from " + Database + ".PatientVisit where DateOfService >= '" + WEEK_START + " 00:00:00' and DateOfService <= '" + WEEK_END + " 23:59:59'";
            Query = "Select COUNT(*) from " + Database + ".PatientVisit a " +
                    "INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID " +
                    "where " +
                    "DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') >= '" + WEEK_START + " 00:00:00' and " +
                    "DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') <= '" + WEEK_END + " 23:59:59' AND " +
                    "b.status=0";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
...
//                        "and  PatientVisit.CreatedDate >= '" + WEEK_START + " 00:00:00' and PatientVisit.CreatedDate <= '" + WEEK_END + " 23:59:59'";
                Query = "Select COUNT(*) from " + Database + ".PatientReg_Details d" +
                        " LEFT JOIN " + Database + ".PatientVisit a ON d.PatientRegId=a.PatientRegId " +
                        "INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID" +
                        " where d.HealthInsuranceChk = 0 \n" +
...
//                        "and  PatientVisit.CreatedDate >= '" + WEEK_START + " 00:00:00' and PatientVisit.CreatedDate <= '" +  + " 23:59:59'";
                Query = "Select COUNT(*) from " + Database + ".PatientReg_Details d" +
                        " LEFT JOIN  " + Database + ".PatientVisit a ON d.PatientRegId=a.PatientRegId  " +
                        "INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID" +
                        " where WorkersCompPolicyChk = 1 \n" +
...
//                Query = "Select COUNT(*) from " + Database + ".PatientReg_Details LEFT JOIN " + Database + ".PatientVisit ON PatientReg_Details.PatientRegId=PatientVisit.PatientRegId " +
//                        "where PatientReg_Details.HealthInsuranceChk = 1 \n" +
//                        "and  PatientVisit.CreatedDate >= '" + MONTH_START + " 00:00:00' and PatientVisit.CreatedDate <= '" + MONTH_END + " 23:59:59'";
                Query = "Select COUNT(*) from " + Database + ".PatientReg_Details d" +
                        " LEFT JOIN " + Database + ".PatientVisit a ON d.PatientRegId=a.PatientRegId " +
                        "INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID" +
                        " where d.HealthInsuranceChk = 1 \n" +
...
//            Query = "Select COUNT(*) from " + Database + ".PatientVisit where DateOfService >= '" + LAST_MONTH_START + " 00:00:00' and DateOfService <= '" + LAST_MONTH_END + " 23:59:59'";
            Query = "Select COUNT(*) from " + Database + ".PatientVisit a " +
                    "INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID " +
                    "where " +
                    "DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') >= '" + LAST_MONTH_START + " 00:00:00' and " +
                    "DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') <= '" + LAST_MONTH_END + " 23:59:59' AND " +
                    "b.status=0";
            ;
...
//            stmt.close();
             ...
             * ---------------------------------------------------------------------------------------------
             */
/*            Query = "Select Id,Concat('Dr.',DoctorsLastName, ', ', DoctorsFirstName) " +
                    "from " + Database + ".DoctorsList where Status = 1";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                Query1 = "Select COUNT(*) from " + Database + ".PatientVisit " +
                        "where  DoctorId = " + rset.getInt(1);
                stmt1 = conn.createStatement();
                rset1 = stmt1.executeQuery(Query1);
            ...
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                Query1 = "Select COUNT(*) from " + Database + ".PatientVisit where  DoctorId = " + rset.getInt(1) + " " +
                        " and  DateofService between '" + MONTH_START + " 00:00:00' and '" + MONTH_END + " 23:59:59' ";
                stmt1 = conn.createStatement();
                rset1 = stmt1.executeQuery(Query1);
                DoctorNames.append("'" + rset.getString(2) + "',");
                if (rset1.next()) {
            ...
            if (ClientId == 27 || ClientId == 29) {
                ...
                Query = " Select SUM(FrVisitedBefore),SUM(FrFamiliyVisitedBefore),SUM(FrInternet),SUM(FrBillboard),SUM(FrGoogle),SUM(FrBuildingSignage),SUM(FrFacebook),SUM(FrLivesNear)," +
                        ...
                        "where CreatedDate between '" + MONTH_START + " 00:00:00' and '" + MONTH_END + " 23:59:59' ";
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                if (rset.next()) {
...
//            stmt.close();
            ...
            while (rset.next()) {
                ...
                _ToEndDate = rset.getString(1);
//                Query1 = "Select COUNT(*) from " + Database + ".PatientVisit  WHERE DateOfService >= '" + _FromEndDate + " 00:00:00' and DateOfService <= '" + _ToEndDate + " 23:59:59'";
                Query1 = "SELECT COUNT(*) FROM " + Database + ".PatientVisit a " +
                        " INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID " +
                        " WHERE " +
                        " DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') >= '" + _FromEndDate + " 00:00:00' AND " +
                        " DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s') <= '" + _ToEndDate + " 23:59:59' AND " +
                        " b.status=0";
                stmt1 = conn.createStatement();
                rset1 = stmt1.executeQuery(Query1);
                while (rset1.next()) {
...
//            Query = "Select COUNT(*) from " + Database + ".PatientVisit where   CreatedDate >= '" + FromDate + " 00:00:00' and CreatedDate <= '" + ToDate + " 23:59:59'";
//            stmt = conn.createStatement();
//            rset = stmt.executeQuery(Query);
//            if (rset.next()) {
...
//                Query = "Select COUNT(*) from " + Database + ".PatientVisit where  DATE_FORMAT(DateOfService,'%Y-%m') = '" + CurrentYear + "-" + Months[i] + "'";
                Query = "Select COUNT(*) from " + Database + ".PatientVisit a " +
                        "INNER JOIN " + Database + ".PatientReg b ON a.PatientRegId=b.ID " +
                        "where  " +
                        "DATE_FORMAT(DATE_FORMAT(a.DateOfService,'%Y-%m-%d %h:%i:%s'),'%Y-%m') = '" + CurrentYear + "-" + Months[i] + "' AND " +
                        "b.status=0";
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                if (rset.next()) {
