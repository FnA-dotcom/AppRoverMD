...
public class PatientVisit22APRIL2021 extends HttpServlet {
...
//      }
            ...
            switch (ActionID) {
                ...
                case "GetPatients":
                    ...
                    this.GetPatients(request, out, conn, context, UserId, Database, ClientId);
                    break;
                case "GetData":
                    supp.Dologing(UserId, conn, request.getRemoteAddr(), ActionID, "Search Patients Visit Get Details", "Get Data for all Matching Old Patients ", ClientId);
    ...
    void GetPatients(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId) {
        try {
            Statement stmt = null;
            ResultSet rset = null;
            String Query = "";
            ...
            Query = "Select Id, MRN, FirstName, MiddleInitial, LastName, DOB, PhNumber  from " + Database + ".PatientReg " +
                    "where status = 0 and FirstName like '%" + Patient + "%' OR LastName like '%" + Patient + "%'  OR " +
                    "MRN like '%" + Patient + "%' OR PhNumber like '%" + Patient + "%'  OR MiddleInitial like '%" + Patient + "%'";
...
////      out.println(Query);
//      while (rset.next()) {
            Query2 = " Select CONCAT(IFNULL(b.Title,''),' ',b.FirstName,' ',b.MiddleInitial,' ',b.LastName), " +
                    " DATE_FORMAT(b.DOB,'%m/%d/%Y'), b.PhNumber, IFNULL(a.ReasonVisit,'-') , " +
                    ...
                    " Left Join " + Database + ".PatientReg b on a.PatientRegId = b.Id where b.ID =  " + PatientId + "" +
        ...
        try {
            ...
            try {
                Query = "UPDATE " + Database + ".PatientReg SET ReasonVisit ='" + NewReasonVisit + "', " +
                        "DateofService = '" + NewDateofService + "', DoctorsName = '" + NewDoctorId + "', sync = 0 " +
