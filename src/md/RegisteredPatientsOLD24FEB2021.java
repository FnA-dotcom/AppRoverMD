...
public class RegisteredPatientsOLD24FEB2021 extends HttpServlet {
    static String DOS = "";
    static String Acct = "";
...
//    conn = Services.getMysqlConn();
        ...
        try {
            ...
            if (ActionID.equals("GetValues")) {
                ...
                this.CashReceipt(request, response, out, conn, context, UserId, Database, ClientId);
    ...
    void CollectPayment_View(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            Query = " SELECT a.PatientMRN, CONCAT(b.Title,' ',b.FirstName,' ',b.MiddleInitial,' ',b.LastName), DATE_FORMAT(b.DOB,'%d-%m-%Y'), " +
                    " IFNULL(DATE_FORMAT(b.DateofService,'%m/%d/%Y %T'),DATE_FORMAT(b.CreatedDate,'%m/%d/%Y %T')) AS DOS,  a.InvoiceNo, DATE_FORMAT(a.CreatedDate, '%m/%d/%Y %T'), a.TotalAmount, a.PaidAmount, a.BalAmount, a.Id  FROM " + Database + ".InvoiceMaster a " + " LEFT JOIN " + Database + ".PatientReg b ON a.PatientMRN = b.MRN  " + " WHERE a.PatientMRN = '" + PatientInvoiceMRN + "' and a.Status = 0 ";
            //out.println(Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
...
//            out.println("<input class=\"btn btn-primary\" type=button name=Back Value=\"  Back  \" onclick=history.back()>");
    ...
    void TransactionReport(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            Query = " Select a.PatientMRN , CONCAT(b.Title,' ',b.FirstName,' ',b.MiddleInitial,' ',b.LastName), a.InvoiceNo, a.TotalAmount, a.PaidAmount,  a.BalAmount, a.RefNo, a.Remarks, DATE_FORMAT(a.CreatedDate, '%m/%d/%Y %T') as PaymentDate, DATE_FORMAT(c.CreatedDate, '%m/%d/%Y %T') as InvoiceDate, " +
                    " CASE WHEN PayMethod = 1 THEN 'Credit Card' WHEN PayMethod = 2 THEN 'Cash' WHEN PayMethod = 3 THEN 'BOLT Device' WHEN PayMethod = 4 THEN 'Cheque' ELSE '' END, IFNULL(PayMethod,0) from " + Database + ".PaymentReceiptInfo a LEFT JOIN " + Database + ".PatientReg b on a.PatientMRN = b.MRN" + " LEFT JOIN " + Database + ".InvoiceMaster c on a.InvoiceNo = c.InvoiceNo where a.PatientMRN = '" + MRN + "'";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
...
//                System.out.println(Query);
    ...
    void CashReceipt(final HttpServletRequest request, final HttpServletResponse response, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
        String PatientMRN = "";
        ...
        try {
            ...
            Query = "Select CONCAT(IFNULL(b.Title,''),' ', IFNULL(b.FirstName,''), ' ', IFNULL(b.MiddleInitial,''), ' ', IFNULL(b.LastName,'')), " +
                    "DATE_FORMAT(a.CreatedDate,'%m/%d/%Y %T'), b.ReasonVisit, a.PaidAmount from " + Database + ".PaymentReceiptInfo a LEFT JOIN " + Database + ".PatientReg b on a.PatientMRN = b.MRN " +
                    "where a.PatientMRN = '" + PatientMRN + "' and a.InvoiceNo = '" + InvoiceNo + "'";
            //out.println(Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            final String outputFilePath = "/sftpdrive/AdmissionBundlePdf/" + DirectoryName + "/" + PatientMRN + InvoiceNo + "_CashReceipt" + ".pdf";
            ...
            for (int i = 1; i <= pdfReader.getNumberOfPages(); ++i) {
                ...
                if (i == 1) {
                    ...
                    pdfContentByte.setFontAndSize(BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.CP1257, BaseFont.EMBEDDED), 14); // set fonts zine and name
                    pdfContentByte.setColorFill(BaseColor.BLACK);
                    pdfContentByte.setTextMatrix(300, 632); // set x and y co-ordinates
                    pdfContentByte.showText(PatientName); // add the text
                    pdfContentByte.endText();

                    pdfContentByte.beginText();
                    pdfContentByte.setFontAndSize(BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.CP1257, BaseFont.EMBEDDED), 14); // set fonts zine and name
                    pdfContentByte.setColorFill(BaseColor.BLACK);
                    pdfContentByte.setTextMatrix(250, 597); // set x and y co-ordinates
                    pdfContentByte.showText(PaymentDate); // add the text
                    pdfContentByte.endText();

                    pdfContentByte.beginText();
                    pdfContentByte.setFontAndSize(BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.CP1257, BaseFont.EMBEDDED), 14); // set fonts zine and name
                    pdfContentByte.setColorFill(BaseColor.BLACK);
                    pdfContentByte.setTextMatrix(200, 512); // set x and y co-ordinates
                    pdfContentByte.showText(ReasonVisitCatagory); // add the text Reason of VIsit Catagory
                    pdfContentByte.endText();

                    pdfContentByte.beginText();
                    pdfContentByte.setFontAndSize(BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.CP1257, BaseFont.EMBEDDED), 14); // set fonts zine and name
                    pdfContentByte.setColorFill(BaseColor.BLACK);
                    pdfContentByte.setTextMatrix(250, 490); // set x and y co-ordinates
                    ...
                    pdfContentByte.setFontAndSize(BaseFont.createFont(BaseFont.TIMES_ROMAN, BaseFont.CP1257, BaseFont.EMBEDDED), 14); // set fonts zine and name
                    pdfContentByte.setColorFill(BaseColor.BLACK);
                    pdfContentByte.setTextMatrix(350, 435); // set x and y co-ordinates
                    pdfContentByte.showText(PaidAmount); // add the text
                    pdfContentByte.endText();
                }
            ...
            //out.println( PatientName + PaymentDate + ReasonVisit);
