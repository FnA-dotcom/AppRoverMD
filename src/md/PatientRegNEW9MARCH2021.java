...
public class PatientRegNEW9MARCH2021 extends HttpServlet {
    ...
    public void handleRequest(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ActionID.equals("GetValues")) {
            ...
            this.EditValues_New(request, out, conn, context, Database);
            ...
            this.EditSave(request, out, conn, context, Database, ClientId);
            ...
            this.EditSave_New(request, out, conn, context, Database);
    ...
    void GetValues(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, String Database) {
        try {
            ...
            Query = "Select Id, PayerId, LTRIM(rtrim(REPLACE(PayerName,'Servicing States','') )) from " + Database + ".ProfessionalPayers where id  in (902,8289,8297,123,127,5800,1259,2700,5978,389,2337,1460,2348,3901,2583,2588,2393,955) AND Status != 100 group by PayerId";//where PayerName like '%Texas%'";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
...
//            }
//                out.println("TEST TABISH " + SelfPayChk + "<br>");
...
//out.println("TEST TABISH " + "789" + "<br>");
...
//out.println("TEST TABISH " + "123" + "<br>");
...
//                out.println("TEST TABISH " + Query + "<br>");
                    ...
                    out.println(str);
                    out.println("Error in MRN HERE: " + e.getMessage() + Query);
                    return;
...
//            out.println("TEST TABISH " + "AFTER CLSS CURRNT DATE" + "<br>");
...
//            out.println(CurrentDate + "<br>");
            ...
            try {
                ...
                MainReceipt.setString(5, "OutPatient");
                MainReceipt.executeUpdate();
                MainReceipt.close();
            } catch (Exception e) {
            ...
            try {
                final PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".PatientReg_Details (PatientRegId,MRN,TravellingChk,TravelWhen,TravelWhere ,TravelHowLong,COVIDExposedChk, " +
                                ...
                                " SympHeadache, SympLossTaste, SympShortBreath) " +
                                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
            ...
            try {
                if (SelfPayChk == 1) {
                    ...
                    if (ClientIndex == 10 || ClientIndex == 15) {
                        try {
                            final PreparedStatement MainReceipt = conn.prepareStatement(
                                    "INSERT INTO " + Database + ".PatientAdmissionBundle(PatientRegId,AdmissionBundle,CreatedDate) VALUES (?,?,now()) ");
                            MainReceipt.setInt(1, PatientRegId);
                    ...
                    final PreparedStatement MainReceipt = conn.prepareStatement(
                            "INSERT INTO " + Database + ".InsuranceInfo(PatientRegId,WorkersCompPolicy,MotorVehAccident,PriInsurance,MemId," +
                                    "GrpNumber,PriInsuranceName,AddressIfDifferent,PrimaryDOB,PrimarySSN,PatientRelationtoPrimary,PrimaryOccupation," +
                                    "PrimaryEmployer,EmployerAddress,EmployerPhone,SecondryInsurance,SubscriberName,SubscriberDOB,MemberID_2," +
                                    "GroupNumber_2,PatientRelationshiptoSecondry,CreatedDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now()) ");
                    MainReceipt.setInt(1, PatientRegId);
            ...
            try {
                final PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".EmergencyInfo (PatientRegId,NextofKinName,RelationToPatient,PhoneNumber,LeaveMessage," +
                                "Address,City,State,Country,ZipCode,CreatedDate) VALUES (?,?,?,?,?,?,?,?,?,?,now()) ");
                MainReceipt.setInt(1, PatientRegId);
            ...
            try {
                PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".ConcentToTreatmentInfo (PatientRegId,PatientSign,Date,Witness,PatientBehalfSign," +
                                "RelativeSign,Date2,Witness2,CreatedDate) \nVALUES (?,?,?,?,?,?,?,?,now()) ");
                MainReceipt.setInt(1, PatientRegId);
            ...
            try {
                final PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".RandomCheckInfo (PatientRegId,ReturnPatient,Google,MapSearch,Billboard,OnlineReview,TV," +
                                "Website,BuildingSignDriveBy,Facebook,School,School_text,Twitter,Magazine,Magazine_text,Newspaper,Newspaper_text," +
                                "FamilyFriend,FamilyFriend_text,UrgentCare,UrgentCare_text,CommunityEvent,CommunityEvent_text,Work_text," +
                                "Physician_text,Other_text,CreatedDate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now()) ");
                MainReceipt.setInt(1, PatientRegId);
...
//            }
            ...
            if (ReasonVisit.trim().toUpperCase().equals("ANTIBODY TEST") || ReasonVisit.trim().toUpperCase().equals("RAPID ANTIGEN TEST")) {
            ...
            System.out.println(ReasonVisit);
            if (ClientId == 27) {
...
//            }
            ...
            try {
                Query = "UPDATE " + Database + ".PatientReg SET Title ='" + Title + "', FirstName = '" + FirstName + "', LastName = '" + LastName + "', MiddleInitial = '" + MiddleInitial + "', " + " DOB = '" + DOB + "', Age = '" + Age + "', Gender = '" + gender + "', Email = '" + Email + "', PhNumber = '" + PhNumber + "', Address = '" + Address + "', City = '" + City + "', " + " State = '" + State + "', Country = '" + Country + "', ZipCode = '" + ZipCode + "', SSN = '" + SSN + "', Occupation = '" + Occupation + "', Employer = '" + Employer + "', " + " EmpContact = '" + EmpContact + "', PriCarePhy = '" + PriCarePhy + "', ReasonVisit = '" + ReasonVisit + "', SelfPayChk = '" + SelfPayChk + "', MaritalStatus = '" + MaritalStatus + "', " +
            ...
            try {
                Query = "Update " + Database + ".PatientReg_Details set TravellingChk = '" + TravellingChk + "',TravelWhen = '" + TravelWhen + "',TravelWhere = '" + TravelWhere + "', " +
                        ...
                        " SympLossTaste = '" + SympLossTaste + "', SympShortBreath = '" + SympShortBreath + "' WHERE PatientRegId = " + ID;
                stmt = conn.createStatement();
                stmt.executeUpdate(Query);
                stmt.close();
            } catch (Exception e) {
