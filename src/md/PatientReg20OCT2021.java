...
public class PatientReg20OCT2021 extends HttpServlet {
...
//                out.println("You are not Authorized to access this page");
            ...
            switch (ActionID) {
                case "GetValues":
                    ...
                    break;
                case "SaveData":
                    SaveData(request, out, conn, context, DatabaseName, UserId, helper);
                    break;
                case "EditValues_New":
    ...
    void GetValues(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String Database, UtilityHelper helper, int facilityIndex) throws FileNotFoundException {
        ...
        try {
            ...
            Query = "Select Id, PayerId, LTRIM(rtrim(REPLACE(PayerName,'Servicing States','') )) from oe_2.ProfessionalPayers " +
                    "where id  in (902,8289,8297,123,127,5800,1259,2700,5978,389,2337,1460,2348,3901,2583,2588,2393,955,64,1545,3646,8589,200,201,202,203,204,205,206,207,3649,5978,8206,4763,3465,3466,3467,3468,41,387,388,389,697,698,4757, 8605, 8606, 8254, 2560, 1663,8607) " +
    ...
    void SaveData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String Database, String UserId, UtilityHelper helper) throws FileNotFoundException {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
        int PatientRegId = 0;
        ...
        try {
            try {
                ...
                if (request.getParameter("ReasonVisitSelect") == null) {
                    ReasonVisitOthers = "";
                } else {
                    ReasonVisitOthers = request.getParameter("ReasonVisitSelect").trim();
                }
            ...
            try {
                PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".PatientReg (ClientIndex,FirstName,LastName ,MiddleInitial,DOB,Age,Gender ,Email,PhNumber ," +
                                ...
                                "Address2, StreetAddress2, EnterBy, EnterIP,RegisterFrom,ViewDate,ReasonVisitOthers) " +
                                "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),?,?,?,?,0,?,?,?,?,?,?,?,?,?,NOW(),?) ");
    ...
    void EditValues_New(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String Database, String UserId, UtilityHelper helper) throws FileNotFoundException {
        ...
        try {
            Query = " Select IFNULL(LastName,'-'), IFNULL(FirstName,'-'), IFNULL(MiddleInitial,'-'), IFNULL(Title,'-'), " +
                    ...
                    "IFNULL(DateofService, '-'), IFNULL(Ethnicity,''), IFNULL(County,''), IFNULL(Address2,''), IFNULL(StreetAddress2,''),IFNULL(ReasonVisitOthers,'') " +
            ...
            if (rset.next()) {
                ...
                ReasonVisitOthers = rset.getString(33);
            }
            rset.close();
            stmt.close();
            ...
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            TitleBuff.append("<option value='' selected disabled>Select Title</option>");
            while (rset.next()) {
            ...
            if (ReasonVisitOthers.equals("COVID")) {//(ReasonVisit.trim().toUpperCase().equals("ANTIBODY TEST") || ReasonVisit.trim().toUpperCase().equals("RAPID ANTIGEN TEST") || ReasonVisit.trim().toUpperCase().equals("COVID PCR") || ReasonVisit.trim().toUpperCase().equals("COVID TESTING") || ReasonVisit.trim().toUpperCase().equals("COVID VACCINE")) {
                ReasonVisitBuff.append("<select class=\"form-control\" id=\"ReasonVisitSelect\" name=\"ReasonVisitSelect\" style=\"color:black;\" onchange=\"DisplayReasonVisitField(this.value);\" required>\n<option value=\"\">Select Reason of Visit</option>\n<option value=\"Others\">Emergency</option>\n<option value=\"COVID\" selected>COVID-19</option>\n<option id=\"OthersforExcel\" value=\"999\">Others</option></select> \n");
            } else if (ReasonVisitOthers.equals("999")) {
                ReasonVisitBuff.append("<select class=\"form-control\" id=\"ReasonVisitSelect\" name=\"ReasonVisitSelect\" style=\"color:black;\" onchange=\"DisplayReasonVisitField(this.value);\" required>\n<option value=\"\">Select Reason of Visit</option>\n<option value=\"Others\" >Emergency</option>\n<option value=\"COVID\" >COVID-19</option>\n<option id=\"OthersforExcel\" value=\"999\" selected>Others</option></select>\n");
            } else {
                ReasonVisitBuff.append("<select class=\"form-control\" id=\"ReasonVisitSelect\" name=\"ReasonVisitSelect\" style=\"color:black;\" onchange=\"DisplayReasonVisitField(this.value);\" required>\n<option value=\"\">Select Reason of Visit</option>\n<option value=\"Others\" selected>Emergency</option>\n<option value=\"COVID\" >COVID-19</option>\n<option id=\"OthersforExcel\" value=\"999\">Others</option></select>\n");
            }

            if (ClientId == 27 || ClientId == 29) {
                if (ReasonVisit.trim().toUpperCase().equals("ANTIBODY TEST") || ReasonVisit.trim().toUpperCase().equals("RAPID ANTIGEN TEST") || ReasonVisit.trim().toUpperCase().equals("COVID PCR") || ReasonVisit.trim().toUpperCase().equals("COVID VACCINE")) {
    ...
    void EditSave_New(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String Database, String UserId, UtilityHelper helper) throws FileNotFoundException {
        ...
        try {
            ...
            try {
                ...
                helper.SendEmailWithAttachment("Error in PatientReg ** (EditSave_New PatineReg Data get ^^" + facilityName + "_PatMRN#: " + MRN + " ##MES#017)", servletContext, ex, "PatientReg", "EditSave_New", conn);
...
/*                Query = " Update " + Database + ".PatientReg_Details set TravellingChk = '" + TravellingChk + "'," +
                ...
                helper.SendEmailWithAttachment("Error in PatientReg ** (EditSave_New update PatientReg_Details ^^" + facilityName + "_PatMRN#: " + MRN + " ##MES#022)", servletContext, ex, "PatientReg", "EditSave_New", conn);
...
/*                        Query = " UPDATE " + Database + ".InsuranceInfo SET WorkersCompPolicy = " + WorkersCompPolicy + ", " +
                        ...
                        try {
                            ...
                            helper.SendEmailWithAttachment("Error in PatientReg ** (EditSave_New Error in Inserting Insurance Info Data ^^" + facilityName + "_PatMRN#: " + MRN + " ##MES#023)", servletContext, ex, "PatientReg", "EditSave_New", conn);
                ...
                helper.SendEmailWithAttachment("Error in PatientReg ** (EditSave_New Error in Updating Insurance Info Table ^^" + facilityName + "_PatMRN#: " + MRN + " ##MES#024)", servletContext, ex, "PatientReg", "EditSave_New", conn);
...
/*                Query = "Update " + Database + ".EmergencyInfo set NextofKinName = '" + NextofKinName + "', " +
                ...
                helper.SendEmailWithAttachment("Error in PatientReg ** (EditSave_New Error in Updating Emergency Info Table ^^" + facilityName + "_PatMRN#: " + MRN + " ##MES#024-AA)", servletContext, ex, "PatientReg", "EditSave_New", conn);
...
/*            Query = "Select dbname from oe.clients where Id = " + ClientIndex;
            ...
            rset.close();
            stmt.close();*/
            if (FirstName.length() > 1 && LastName.length() > 1 && DOB.length() > 1) {
                int PatientFound = 0;
                String FoundMRN = "";
            ...
            Services.DumException("CheckPatient^^Facility : " + facilityName + " ##MES#00001", "PatientReg ", request, e);
            helper.SendEmailWithAttachment("Error in PatientReg Facility : " + facilityName + " ** (CheckPatient Error in Main Catch ^^ ##MES#00001)", servletContext, e, "PatientReg", "CheckPatient", conn);
