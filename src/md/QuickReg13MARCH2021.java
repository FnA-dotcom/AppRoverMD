...
public class QuickReg13MARCH2021 extends HttpServlet {
    ...
    public void handleRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ActionID.equals("GetValues")) {
            ...
            GetValues(request, out, conn, context, UserId, Database, FontColor);
            ...
            SaveData(request, out, conn, context, UserId, response, Database, FontColor);
            ...
            Victoria_2(request, out, conn, context, UserId, response, Database, FontColor);
        }
        try {
            conn.close();
    ...
    void GetValues(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, String FontColor) {
        try {
            ...
            String ClientIndexS = request.getParameter("ClientIndex").trim();
            String ClientId = "";
            String QRF_name = "";
            ...
            rset.close();
            stmt.close();
            Query = "Select Id, CONCAT(IFNULL(DoctorsLastName,''),', ',IFNULL(DoctorsFirstName,'')) from " + Database + ".DoctorsList " +
                    "where Status = 1 order by DoctorsLastName";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next())
            ...
            Parser.GenerateHtml(out, String.valueOf(String.valueOf(Services.GetHtmlPath(getServletContext()))) + "Forms/" + QRF_name);
        } catch (Exception exception) {
        }
    }

    void SaveData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, HttpServletResponse response, String Database, String FontColor) {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
        int PatientRegId = 0;
        ...
        try {
            ...
            stmt.close();
            try {
                PreparedStatement MainReceipt = conn.prepareStatement("INSERT INTO " + Database + ".PatientReg (ClientIndex,FirstName,LastName ,MiddleInitial,DOB,Age,Gender ,Email,PhNumber ,Address,City ,State,Country,ZipCode,SSN,Occupation ,Employer ,EmpContact,PriCarePhy,ReasonVisit,SelfPayChk,CreatedDate,Title, MaritalStatus,CreatedBy, MRN, Status, DoctorsName, DateofService, ExtendedMRN, Address2, StreetAddress2, County) \nVALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),?,?,?,?,0,?,?,?,?,?,?) ");
            ...
            try {
                PreparedStatement MainReceipt = conn.prepareStatement("INSERT INTO " + Database + ".PatientVisit(MRN,PatientRegId,ReasonVisit,VisitNumber,DoctorId,DateofService,CreatedDate,CreatedBy) \nVALUES (?,?,?,1,NULL,?,now(),?) ");
            ...
            try {
                PreparedStatement MainReceipt = conn.prepareStatement("INSERT INTO " + Database + ".PatientReg_Details (PatientRegId,MRN,TravellingChk,TravelWhen,TravelWhere ,TravelHowLong,COVIDExposedChk,  SympFever,SympBodyAches ,SympSoreThroat,SympFatigue , SympRash,SympVomiting ,SympDiarrhea,SympCough,SympRunnyNose,SympNausea,SympFluSymptoms ,SympEyeConjunctivitis, Race, CovidExpWhen, SpCarePhy) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ");
            ...
            Parser.SetField("ClientIndex", String.valueOf(ClientIndex));
            Parser.SetField("FontColor", String.valueOf(FontColor));
            System.out.println(String.valueOf(Services.GetHtmlPath(getServletContext())));
            Parser.GenerateHtml(out, String.valueOf(String.valueOf(Services.GetHtmlPath(getServletContext()))) + "Exception/Message.html");
        } catch (Exception e) {
    ...
    void Victoria_2(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, HttpServletResponse response, String Database, String FontColor) {
        try {
            Statement stmt = null;
            ResultSet rset = null;
            String Query = "";
            ...
            try {
                ...
                Email = ConfirmEmail;
                PreparedStatement MainReceipt = conn.prepareStatement(" INSERT INTO " + Database + ".PatientReg (ClientIndex,FirstName,LastName , MiddleInitial,DOB,Age,Gender ,Email,PhNumber ,Address,City ,State,Country,ZipCode,SSN,Occupation ,Employer ,EmpContact, PriCarePhy,ReasonVisit,SelfPayChk,CreatedDate,Title, MaritalStatus,CreatedBy, MRN, Status, ExtendedMRN, County, DoctorsName) \n VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),?,?,?,?,0,?,?,?) ");
                MainReceipt.setInt(1, ClientIndex);
                MainReceipt.setString(2, FirstName);
                MainReceipt.setString(3, LastName);
                MainReceipt.setString(4, MiddleInitial);
                ...
                Services.DumException("QuickReg", "Victoria_2 Error 2- Insertion PatientReg Table : ", request, e, getServletContext());
                return;
            }
            try {
                Query = "Select max(ID) from " + Database + ".PatientReg ";
                ...
                Services.DumException("QuickReg", "Victoria_2 Error 3- : ", request, e, getServletContext());
                return;
            }
            try {
                ...
                Services.DumException("QuickReg", "Victoria_2 Error 3.1 Insertion in table PatientVisit- : ", request, e, getServletContext());
                return;
            }
            try {
                ...
                Services.DumException("QuickReg", "Victoria_2 Error 4- Insertion PatientReg_Details Table : ", request, e, getServletContext());
                return;
            }
            if (WorkersCompPolicyChk.equals("1"))
                try {
                    ...
                    Services.DumException("QuickReg", "Victoria_2 Error 5- Insertion Patient_WorkCompPolicy Table : ", request, e, getServletContext());
                    return;
                }
            if (MotorVehicleAccidentChk.equals("1"))
                try {
            ...
            if (HealthInsuranceChk.equals("1"))
                try {
                    ...
                    Services.DumException("QuickReg", "Victoria_2Error 7- Insertion Patient_HealthInsuranceInfo Table : ", request, e, getServletContext());
                    return;
                }
            try {
                PreparedStatement MainReceipt = conn.prepareStatement(" INSERT INTO " + Database + ".MarketingInfo (PatientRegId,MFFirstVisit,MFReturnPat,MFInternetFind,Facebook,MapSearch,GoogleSearch,VERWebsite,WebsiteAds,OnlineReviews,Twitter,LinkedIn,EmailBlast,YouTube,TV,Billboard,Radio,Brochure,DirectMail,CitizensDeTar,LiveWorkNearby,FamilyFriend,FamilyFriend_text,UrgentCare,UrgentCare_text,NewspaperMagazine,NewspaperMagazine_text,School,School_text,Hotel,Hotel_text,MFPhysician,CreatedDate) \n VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,now()) ");
                MainReceipt.setInt(1, PatientRegId);
                ...
                Services.DumException("PatientReg2", "SaveDataVictoriaError 8- Insertion MarketingInfo Table", request, e, getServletContext());
                return;
            }
            ...
            Services.DumException("QuickReg", "Victoria_2", request, ex, getServletContext());
        }
    }
    ...
    private String InsertCOVIDReg(HttpServletRequest request, HttpServletResponse response, PrintWriter out, Connection conn, String PatientRegId) throws JsonProcessingException {
        ...
        try {
            ...
            Services.DumException("QuickReg", "InsertCOVIDRegError in Getting Data from PatientReg table", request, e, getServletContext());
        }
        ...
        try {
            ...
            responseJSON.put("IsValidDOB", Boolean.valueOf(true));
            ...
            String Mask = "";
            URL url = new URL("https://victoriacovid.com/api/CovidPatient/CreatePatient/?UserId=1");
            ...
            Services.DumException("QuickReg", "InsertCOVIDReg 0", request, e, getServletContext());
            return "0";
        }
        return reply;
    }
