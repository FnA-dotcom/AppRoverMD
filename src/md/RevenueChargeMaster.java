...
public class RevenueChargeMaster extends HttpServlet {
    ...
    public void handleRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            try {
                ...
                e.printStackTrace();
                helper.SendEmailWithAttachment("Error in RevenueChargeMaster ** (handleRequest -- SqlException)", context, e, "RevenueChargeMaster", "handleRequest", conn);
                Services.DumException("RevenueChargeMaster", "Handle Request", request, e, getServletContext());
                Parsehtm Parser = new Parsehtm(request);
    ...
    void GetInput(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        StringBuffer RevenueList = new StringBuffer();
        int SNo = 1;
        try {
            Query = "Select Id, ifnull(RevCode,''),ifnull(Price,'') from " + Database + ".RevCodeChargeMaster  where Status = 0 order by CreatedDate DESC ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);

            while (rset.next()) {

                RevenueList.append("<tr>");
                RevenueList.append("<td align=left>" + SNo + "</td>\n");
                RevenueList.append("<td align=left>" + rset.getString(2) + "</td>\n");
                RevenueList.append("<td align=left>" + rset.getString(3) + "</td>\n");

                RevenueList.append("<td align=left><i data-toggle=\"modal\" data-target=\"#exampleModalCenter\"  id=\"edit\" class=\"fa fa-edit\" onClick=\"editRow(" + rset.getInt(1) + ")\"></i></td>\n");
                RevenueList.append("</tr>");
                SNo++;
            }
            ...
            Parser.SetField("RevenueList", String.valueOf(RevenueList));
            Parser.SetField("UserId", String.valueOf(UserId));
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Forms/RevenueChargeMaster.html");
    ...
    void InsertData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        String RevCode = request.getParameter("RevCode").trim();
...
//    String DBName = request.getParameter("FacilityName").trim();
        ...
        try {
            ...
            PreparedStatement MainReceipt = conn.prepareStatement(
                    "INSERT INTO " + Database + ".RevCodeChargeMaster (RevCode,Price,CreatedBy,Status,CreatedDate) VALUES (?,?,?,0,now())");
            MainReceipt.setString(1, RevCode);
            ...
            helper.SendEmailWithAttachment("Error in RevenueChargeMaster ** (ShowReport)", servletContext, e, "RevenueChargeMaster", "ShowReport", conn);
            Services.DumException("ShowReport", "RevenueChargeMaster ", request, e);
            Parsehtm Parser = new Parsehtm(request);
            Parser.SetField("FormName", "ManagementDashboard");
...
//            System.out.println("FacIdx is----->"+FacIdx);
            ...
            for (i = 1; i <= totalrows; i++) {
                ...
                Price = formatter.formatCellValue(row.getCell(0));
                RevCode = formatter.formatCellValue(row.getCell(1));

                if (RevCode == "" || RevCode == null) {
                    EmptyFlag = 1;
                ...
                Matcher RevCodematcher = p1.matcher(RevCode);
                boolean CptSpecialCharacterFound = RevCodematcher.find();
                System.out.println("CptSpecialCharacterFound" + RevCodematcher);
                if (CptSpecialCharacterFound == false) {
                    PatternFlag = 1;
                }
                Matcher PriceMatcher = p2.matcher(Price);
                boolean RevCodeSpecialCharacterFound = PriceMatcher.find();
                if (RevCodeSpecialCharacterFound == false) {
                    System.out.println("RevCodeSpecialCharacterFound" + PriceMatcher);
                    PatternFlag = 1;
                }
                SMSList.append("<tr>");
                SMSList.append("<td align=left>" + RevCode + "</td>");
...
//            Parser.SetField("Facility", String.valueOf(Facility));
            ...
            Parser.GenerateHtml(out, Services.GetHtmlPath(servletContext) + "Forms/sendRevenueFile.html");
...
//        String CPT[] = {request.getParameter("CPT").trim()};
        String RevCode = null;//{request.getParameter("CPT").trim()};
...
//        String CPT = request.getParameter("CPT").trim();
        ...
        try {
            for (int i = 1; i <= totalrows; i++) {
                ...
                org.apache.poi.ss.usermodel.Row row = sheet.getRow(i);
                RevCode = formatter.formatCellValue(row.getCell(0));
                Price = formatter.formatCellValue(row.getCell(1));
                System.out.println("her is CPT array" + Price);

                PreparedStatement MainReceipt = conn.prepareStatement(
                        "INSERT INTO " + Database + ".RevCodeChargeMaster (FileName,CreatedBy,RevCode,Price,Status,CreatedDate) VALUES (?,?,?,?,0,now())");
                MainReceipt.setString(1, FileName);

                MainReceipt.setString(2, UserId);

                MainReceipt.setString(3, RevCode);
    ...
    void UpdateData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        try {
            ...
            PreparedStatement MainReceipt = conn.prepareStatement(
                    "Update " + Database + ".RevCodeChargeMaster set RevCode =?,Price =?,UpdatedBy=?,Status=?,UpdatedDate =now()  where Id =" + Id);
            MainReceipt.setString(1, RevCode);
            MainReceipt.setString(2, Price);
            MainReceipt.setString(3, UserId);
    ...
    void EditData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        try {
            Query = "Select  RevCode,Price,Status From " + Database + ".RevCodeChargeMaster where Id = " + Id;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
                RevCode = rset.getString(1);
            ...
            rset.close();
            stmt.close();
            out.println(RevCode + "|" + Price + "|" + String.valueOf(Status) + "|" + String.valueOf(Id));
    ...
    void FacilityData(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId, UtilityHelper helper) throws FileNotFoundException {
        ...
        int SNo = 1;
        try {
            Query = "Select a.Id, ifnull(a.CPTCode,''),ifnull(a.RevCode,''),ifnull(b.name,'') from " + DBName + ".RevenueChargeMaster a left join oe.clients b on a.FacilityName = b.dbname  where a.Status = 0 order by a.CreatedDate DESC ";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
