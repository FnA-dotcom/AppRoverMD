...
public class PatientUpdateInfo_encryption extends HttpServlet {
        ...
        try {
            ...
            switch (ActionID) {
                case "GetInput":
                    ...
                    GetInput(request, out, conn, context, UserId, DatabaseName, FacilityIndex, helper, rsa);
                    break;
                case "AddNotes":
    ...
    private void GetInput(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId, UtilityHelper helper, RSA rsa) throws FileNotFoundException {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
        ...
        try {
            ...
            Query = "SELECT IFNULL(Title,''),IFNULL(FirstName,''),IFNULL(MiddleInitial,''),IFNULL(LastName,''), IFNULL(DOB,''), IFNULL(PhNumber,''), " +
...
//            out.println(Query);
            ...
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
                if (Integer.parseInt(rset.getString(7)) > 311240) {
                    Title = rset.getString(1);
                    if (!rset.getString(2).equals("")) {
                        FName = RSA.decrypt(Base64.getDecoder().decode(rset.getString(2).getBytes()), FILE_ADDRESS);
                    } else {
                    ...
                    if (!rset.getString(3).equals("")) {
                        Middle = RSA.decrypt(Base64.getDecoder().decode(rset.getString(3).getBytes()), FILE_ADDRESS);
                    } else {
                        Middle = rset.getString(3);
                    }

                    if (!rset.getString(5).equals("")) {
                        DOB = RSA.decrypt(Base64.getDecoder().decode(rset.getString(5).getBytes()), FILE_ADDRESS);
                    } else {
                        DOB = rset.getString(5);
                    }

                    Title = Title + " " + FName + " " + Middle + " " + LName;
                } else {
                    Title = rset.getString(1);
                    FName = rset.getString(2);
                    LName = rset.getString(3);
                    Middle = rset.getString(4);
                    DOB = rset.getString(5);
                    Title = Title + " " + FName + " " + LName + " " + Middle;
                }
                PatientName = Title;
                PhNumber = rset.getString(6);
                MRN = rset.getString(7);
                ReasonVisit = rset.getString(8);
                DOS = rset.getString(9);
                ...
                DoctorsId = rset.getString(17);
                SelfPayChk = rset.getInt(18);
            }
            rset.close();
            stmt.close();
...
//                NotesList.append("</tr>\n");
            ...
            if (ClientId == 9) {
                ...
                CDRList.append("<thead style=\"color:black;\">\n<tr>\n<th >MRN</th>\n<th >Acct#</th>\n<th >Patient Name</th>\n<th >Date of Birth</th>\n<th >Number</th>\n<th >Reason of Visit</th>\n<th >Date of Service</th>\n<th >COVID Status</th>\n</tr>\n</thead>\n<tbody style=\"color:black;\">\n");
                Query = " Select  IFNULL(b.Title,''),IFNULL(b.FirstName,''),IFNULL(b.MiddleInitial,''),IFNULL(b.LastName,''), " +
                        "IFNULL(b.DOB,''), IFNULL(b.PhNumber,''),  IFNULL(a.ReasonVisit,'-'), " +
                        "IFNULL(DATE_FORMAT(a.DateofService,'%m/%d/%Y %T'),DATE_FORMAT(b.CreatedDate,'%m/%d/%Y %T')), " +
                        "CASE WHEN b.COVIDStatus = '' THEN 'NONE' WHEN b.COVIDStatus = 1 THEN 'POSITIVE' WHEN b.COVIDStatus = 0 THEN 'NEGATIVE'  " +
                        "WHEN b.COVIDStatus = -1 THEN 'NONE' ELSE 'UN-EXAMINED' END , a.MRN, b.Status, a.VisitNumber, a.Id " +
                        "from " + Database + ".PatientVisit a  " +
                ...
                while (rset.next()) {
                    ...
                    if (Integer.parseInt(MRN) > 311240) {
                        Title = rset.getString(1);
                        if (!rset.getString(2).equals("")) {
                            FName = RSA.decrypt(Base64.getDecoder().decode(rset.getString(2).getBytes()), FILE_ADDRESS);
                        } else {
                    ...
                    CDRList.append("<tr>");
                    CDRList.append("<td align=left>" + rset.getString(7) + "</td>\n");
                    CDRList.append("<td align=left>VN-" + MRN + "-" + rset.getString(12) + "</td>\n");
                    CDRList.append("<td align=left>" + Title + "</td>\n");
