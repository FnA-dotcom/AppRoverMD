...
public class PatientUpdateInfo_OLD_ extends HttpServlet {
...
//                NotesList.append("</tr>\n");
            ...
            Query = "Select Count(*) from " + Database + ".Patient_AdditionalInfo where PatientRegId = " + ID + " and VisitId = '" + VisitId + "'";
            //System.out.println(Query);
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            if (rset.next()) {
            ...
            if (FoundAddInfo > 0) {
                ...
                Query = "Select IFNULL(AdditionalInfoSelect,0), IFNULL(DATE_FORMAT(CovidTestDate,'%Y-%m-%d'),''), IFNULL(PatientCatagory,0), IFNULL(RefName,''), IFNULL(ReasonLeaving,''), " +
                        "IFNULL(RefPhysician,''), IFNULL(RefSourceName,''), IFNULL(PatientStatus,''), IFNULL(CovidTestNo,'')," +
                        "CASE WHEN CovidStatus = '' THEN 'NONE' WHEN CovidStatus = 1 THEN 'POSITIVE' WHEN CovidStatus = 0 THEN 'NEGATIVE'  WHEN CovidStatus = -1 THEN 'NONE' ELSE 'NONE' END  from " + Database + ".Patient_AdditionalInfo where PatientRegId = " + ID + " and VisitId = '" + VisitId + "'";
                //out.println(Query);"
                stmt = conn.createStatement();
                rset = stmt.executeQuery(Query);
                if (rset.next()) {
...
//                    CDRList.append("<th >Claims (Professional)</th>\n");
                }
                ...
                while (rset.next()) {
                    ...
                    rset2.close();
                    stmt2.close();
                    Query2 = "Select COUNT(*) from " + Database + ".ClaimInfoMaster where Status = 0 and PatientRegId = " + ID + " and VisitId = " + rset.getInt(10) + " and ClaimType = 1";
                    stmt2 = conn.createStatement();
                    rset2 = stmt2.executeQuery(Query2);
                    if (rset2.next()) {
                    ...
                    rset2.close();
                    stmt2.close();
                    Query2 = "Select COUNT(*) from " + Database + ".ClaimInfoMaster where Status = 0 and PatientRegId = " + ID + " and VisitId = " + rset.getInt(10) + " and ClaimType = 2";
                    stmt2 = conn.createStatement();
                    rset2 = stmt2.executeQuery(Query2);
                    if (rset2.next()) {
                    ...
                    if (UserType == 7) {
                        ...
                        //<a class='btn-sm btn btn-primary' href=/md/md.RegisteredPatients?ActionID=InvoicePdf&PatientMRN=" + rset.getString(1) + "&InvoiceNo=" + rset.getString(5) + ">View</a>\n");
//                        CDRList.append("<td align=left><button type=\"button\" class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" onclick=\"ClaimAddInfo(" + rset.getInt(10) + ", "+ID+", `VN-"+MRN+"-"+rset.getString(9)+"`, 1)\">Institutional</button></td>\n");
//                        CDRList.append("<td align=left><button type=\"button\" class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" onclick=\"ClaimAddInfo(" + rset.getInt(10) + ", "+ID+", `VN-"+MRN+"-"+rset.getString(9)+"`, 2)\">Professional</button></td>\n");
                        CDRList.append("</td>");
                    }
                    CDRList.append("</tr>");
                }
                ...
                while (rset.next()) {
                    ...
                    if (UserType == 7) {
                        CDRList.append("<td align=left width=20%>");
                        CDRList.append("<a class='btn-sm btn btn-primary claim' href=\"/md/md.AddInfo?ActionID=Addinfo&VisitId=" + rset.getInt(10) + "&PatientRegId=" + ID + "&AcctNo=VN-" + MRN + "-" + rset.getString(9) + "&ClaimType=1\" target=\"_blank\">Institutional<span class=\"badge badge-pill badge-danger\">" + ClaimCountInstitutional + "</span></a>");
...
//                        CDRList.append("<td align=left><button type=\"button\" class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" onclick=\"ClaimAddInfo(" + rset.getInt(10) + ", "+ID+", `VN-"+MRN+"-"+rset.getString(9)+"`,1)\">Intitutional</button></td>\n");
//                        CDRList.append("<td align=left><button type=\"button\" class=\"waves-effect waves-light btn btn-outline btn-rounded btn-info mb-5\" onclick=\"ClaimAddInfo(" + rset.getInt(10) + ", "+ID+", `VN-"+MRN+"-"+rset.getString(9)+"`,2)\">Professional</button></td>\n");
                        CDRList.append("</td>");
                    }
                    CDRList.append("</tr>");
...
//                CDRList.append("<td align=left>" + rset.getString(6) + "</td>\n");
            ...
            helper.SendEmailWithAttachment("Error in PatientUpdateInfo ** (GetInput)", servletContext, e, "PatientUpdateInfo", "GetInput", conn);
...
//                out.println(Query);
            while (rset.next()) {
                ...
                NotesList.append("<td align=left>" + decode(rset.getString(1)) + "</td>\n");
                NotesList.append("<td align=left>" + rset.getString(2) + "</td>\n");
                NotesList.append("<td align=left>" + rset.getString(4) + "</td>\n");
                NotesList.append("<td align=left>" + rset.getString(3) + "</td>\n");
                NotesList.append("<td align=left ><i class=\"fa fa-trash\" onclick=\"DeleteNote(" + rset.getInt(5) + ")\"></i></td>\n");
...
//            NotesList.append("</tbody></table></div>");
    ...
    private void AddAdditionalInfo(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, String UserId, String Database, int ClientId) {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
        ...
        try {
            ...
            Query = "Select COUNT(*) from " + Database + ".Patient_AdditionalInfo where PatientRegId = " + PatientRegId + " and VisitId = '" + VisitId + "'";
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
//            System.out.println(Query);
            while (rset.next()) {
