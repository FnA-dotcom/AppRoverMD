...
public class RegisteredPatients_old extends HttpServlet {
    static String DOS = "";
    static String Acct = "";
        ...
        try {
            ...
            if (PatientCount > 0) {
                ...
                while (rset.next()) {
                    ...
                    CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=ShowHistory&ID=" + rset.getInt(6) + ">Show History</a></td>\n");
//          CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=UpdateAgeVictoria>Show History</a></td>\n");
//          CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=EnterDateinPatientVisit>Show History</a></td>\n");
                    if (ClientId == 9) {
                    ...
                    CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=DeActivePatient&ID=" + rset.getInt(6) + ">De-Activate Patient</a></td>\n");
//          CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=ReActivePatient&ID=" + rset.getInt(6) + ">Re-Activate Patient</a></td>\n");
                    CDRList.append("</tr>");
                    ++SNo;
                }
                rset.close();
...
//    final String Path = "/opt/apache-tomcat-7.0.65/webapps/orange_2/Attachment";
        ...
        String UploadPath = "";
        UploadPath = String.valueOf(String.valueOf(Path)) + "/";
        ...
        try {
            ...
            key = (DocumentType = DocumentType.substring(4));
            final String[] PatientInfo = PatientId.split("\\,");
            PatientRegId = (PatientInfo[0]);
            ...
            try {
                final PreparedStatement MainReceipt = conn.prepareStatement("Insert into " + Database + ".PatientDocUpload (PremisisId, PatientRegId, PatientMRN, PatientName," + " UploadDocumentName, FileName, CreatedBy, CreatedDate, DocumentType) values (?,?,?,?,?,?,?,now(),?) ");
                MainReceipt.setInt(1, PremisisId);
                MainReceipt.setInt(2, Integer.parseInt(PatientRegId));
...
//        System.out.println("Inside Document Type tab here:");
//        try {
//
//          if (PremisisId == 9) {
//            clientdirectory = 14;
    ...
    void ViewDocuments(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, final HttpServletResponse response, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            while (rset.next()) {
                ...
                CDRList.append("<td align=left>" + rset.getString(3) + "</td>\n");
                CDRList.append("<td align=left>" + rset.getString(5) + "</td>\n");
                CDRList.append("<td align=left><a href=https://app.rovermd.com:8443/md/md.RegisteredPatients_old?ActionID=download_direct&fname=" + rset.getString(4) + "&path=" + DocumentPath + " target=\"_blank\">" + rset.getString(4) + "</a></td>\n");
                CDRList.append("</tr>");
                ++SNo;
            }
            rset.close();
...
//        CDRList.append("<td align=left><a href=" + DocumentPath + " target=\"_blank\">" + "ViewInvoice_" + rset.getString(1) + "</a></td>\n");
                CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=InvoicePdf&PatientMRN=" + rset.getString(1) + ">" + "ViewInvoice_" + rset.getString(1) + "</a></td>\n");
                CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=PayNow&InvoiceNo=" + rset.getString(5) + ">PayNow</a></td>\n");
                CDRList.append("<td align=left><a href=/md/md.RegisteredPatients_old?ActionID=DeActiveInvoice&InvoiceMasterID=" + rset.getString(10) + "&InvoiceNo=" + rset.getString(5).trim() + ">De-Activate Invoice</a></td>\n");
                CDRList.append("</tr>");
                ++SNo;
            }
            rset.close();
...
//      System.out.println(Query);
            ...
            PatientList.append("<option value=-1> Please Select Below Patient </option>");
            while (rset.next()) {
                PatientList.append("<option class=Inner value=\"" + rset.getString(8) + "\">" + rset.getString(2) + " | " + rset.getString(3) + " | " + rset.getString(4) + " | " + rset.getString(5) + " | " + rset.getString(6) + " | " + rset.getString(7) + " </option>");
            }
            rset.close();
            stmt.close();
            PatientList.append("</select>");
