...
public class RegisteredPatients2 extends HttpServlet {
    static String DOS = "";

    static String Acct = "";

    static String printabledate = "";
    Integer View_Patient_index = 5;
    Integer Upload_Documents_index = 6;
    Integer ViewDocuments_index = 7;
    Integer AddDoctor_index = 9;
    Integer AddAllergyInfo_index = 10;
    Integer CreateInvoiceInput_index = 12;
    Integer CollectPayment_index = 13;
    ...
    public void handleRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            } else if (ActionID.equals("DeleteIDs")) {
                supp.Dologing(UserId, conn, request.getRemoteAddr(), ActionID, "Patient Delete Document", "Patient Document is Deleted", FacilityIndex);
                DeleteIDs(request, out, conn, context, response, UserId, DatabaseName, FacilityIndex);
            } else if (ActionID.equals("CreateInvoiceInput")) {
                supp.Dologing(UserId, conn, request.getRemoteAddr(), ActionID, "Create Invoice Option", "Invoice Creation Add Diseases", FacilityIndex);
                CreateInvoiceInput(request, out, conn, context, response, UserId, DatabaseName, FacilityIndex, UserType);
...
//style="color:white"
            ...
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
                DOS.append("<option  value=" + rset.getInt(1) + "> " + rset.getString(2) + " | " + rset.getString(3) + " </option>");
            ...
            Query = "SELECT b.MRN, b.FirstName, a.UploadDocumentName, a.FileName, IFNULL(DATE_FORMAT(b.DateofService,'%m/%d/%Y %T')," +
                    " DATE_FORMAT(b.CreatedDate,'%m/%d/%Y %T')), a.Id," +
                    " DATE_FORMAT(c.DateofService,'%m/%d/%Y %T')," +
                    " DATE_FORMAT(a.CreatedDate,'%m/%d/%Y %T') " +
                    " FROM " + Database + ".PatientDocUpload a  " +
                    " LEFT JOIN " + Database + ".PatientReg b ON a.PatientRegId = b.ID " +
                    " INNER JOIN " + Database + ".PatientVisit c ON a.VisitIdx = c.Id " +
                    " WHERE a.Status = 0 and b.status=0 and a.PatientRegId = " + PatientId + " " +
...
//                CDRList.append("<td align=left>" + rset.getString(5) + "</td>\n");
    ...
    void PatientsDocUpload_Save(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, HttpServletResponse response, String Database, int ClientId) {
        ...
        String UploadPath = String.valueOf(Path) + "/";
...
//                }
                ...
                } else if (key.startsWith("VisitNo")) {
...
//            out.println(Query);
    ...
    void DeleteIDs(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, HttpServletResponse response, String UserId, String Database, int ClientId) {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
        String Idx = request.getParameter("Idx").trim();
        int PatId = Integer.parseInt(request.getParameter("ID").trim());

        PreparedStatement ps = null;

        if (Idx.equals("IS")) { // ID
            try {
                ps = conn.prepareStatement("UPDATE " + Database + ".PatientReg SET IDFront_Status = 1 WHERE ID=" + PatId);
                ps.executeUpdate();
                out.println("1");
            } catch (SQLException e) {
                System.out.println("IDFront Update error " + e.getMessage());
            }
        } else if (Idx.equals("IB")) { // Insurance Back
            try {
                ps = conn.prepareStatement("UPDATE " + Database + ".PatientReg SET InsuranceBack_Status = 1 WHERE ID=" + PatId);
                ps.executeUpdate();
                out.println("1");
            } catch (SQLException e) {
                System.out.println("InsuranceBack Update error " + e.getMessage());
            }
        } else if (Idx.equals("IF")) { // Insurance Front
            try {
                ps = conn.prepareStatement("UPDATE " + Database + ".PatientReg SET InsuranceFront_Status = 1 WHERE ID=" + PatId);
                ps.executeUpdate();
                out.println("1");
            } catch (SQLException e) {
                System.out.println("InsuranceFront Update error " + e.getMessage());
            }
        }
    }

    void CreateInvoiceInput(HttpServletRequest request, PrintWriter out, Connection conn, ServletContext servletContext, HttpServletResponse response, String UserId, String Database, int ClientId, int UserType) {
        Statement stmt = null;
        ResultSet rset = null;
        String Query = "";
...
/*            Query = " SELECT a.PatientMRN, CONCAT(IFNULL(b.Title,''),' ',IFNULL(b.FirstName,''),' ',IFNULL(b.MiddleInitial,''),' ',IFNULL(b.LastName,'')), DATE_FORMAT(b.DOB,'%d-%m-%Y')," +
                    "IFNULL(DATE_FORMAT(b.DateofService,'%m/%d/%Y %T'),DATE_FORMAT(b.CreatedDate,'%m/%d/%Y %T')) AS DOS,  a.InvoiceNo, " +
                    "DATE_FORMAT(a.CreatedDate, '%m/%d/%Y %T'), a.TotalAmount, a.PaidAmount, a.BalAmount, a.Id, IFNULL(a.Discount,'0.00')  " +
            ...
            Query = "SELECT a.PatientMRN, CONCAT(IFNULL(b.Title,''),' ',IFNULL(b.FirstName,''),' ',IFNULL(b.MiddleInitial,''),' ',IFNULL(b.LastName,'')) as PatName,\n" +
                    "DATE_FORMAT(b.DOB,'%d-%m-%Y') AS DOB,\n" + //3
                    "IFNULL(DATE_FORMAT(b.DateofService,'%m/%d/%Y %T'),DATE_FORMAT(b.CreatedDate,'%m/%d/%Y %T')) AS DOS, a.InvoiceNo, \n" +//5
                    "DATE_FORMAT(a.CreatedDate, '%m/%d/%Y %T'), a.TotalAmount, a.PaidAmount, a.BalAmount, a.Id, IFNULL(a.Discount,'0.00') ,\n" +//11
                    "DATE_FORMAT(c.DateofService,'%m/%d/%Y %T') AS NDOS,c.Id AS VisitIdx \n" +//13
                    " FROM " + Database + ".InvoiceMaster a " +
                    " INNER JOIN  " + Database + ".PatientReg b ON a.PatientMRN = b.MRN  \n" +
                    " INNER JOIN " + Database + ".PatientVisit c ON a.VisitID = c.Id \n" +
                    " WHERE a.PatientMRN = " + PatientInvoiceMRN + " AND a.Status = 0 and b.status=0 " +
...
//                CDRList.append("<td align=left>" + rset.getString(3) + "</td>\n");
            ...
            Query = "SELECT Id, ReasonVisit,IFNULL(DATE_FORMAT(DateofService,'%m/%d/%Y %T'),DATE_FORMAT(CreatedDate,'%m/%d/%Y %T')) " +
                    "FROM " + Database + ".PatientVisit WHERE MRN = " + PatientInvoiceMRN;
            stmt = conn.createStatement();
            rset = stmt.executeQuery(Query);
            while (rset.next()) {
        ...
        try {
            ...
            try {
                Query = " Select CONCAT(IFNULL(a.FirstName,''), ' ', IFNULL(a.MiddleInitial,''), ' ', IFNULL(a.LastName,'')), " +
                        ...
                        " DATE_FORMAT(a.CreatedDate,'%m/%d/%Y') as DOS, IFNULL(DATE_FORMAT(a.DOB,'%Y-%m-%d'),'') " +
...
//            datatable1.addCell(new Paragraph("MRN: " + PatientMRN + "         DOS: " + DOS, normfont));
            ...
            datatable2.addCell(new Paragraph("", normfont3));
            datatable2.addCell(new Paragraph(nDOS, normfont3));
            datatable2.addCell(new Paragraph("", normfont3));
            datatable2.setDefaultColspan(1);
            datatable2.setDefaultHorizontalAlignment(0);
            datatable2.addCell(new Paragraph("", normfont3));
            ...
            datatable4.setDefaultCellBorder(15);
            datatable4.addCell(new Paragraph(String.valueOf(numFormat.format(TotalAmount - PaidAmount)), normfont3));//here we have changed
            datatable4.setDefaultCellBorder(0);
            datatable4.addCell(new Paragraph("", MainHeading));
            ...
            datatable5.addCell(new Paragraph("Sign: ___________________", normfont3));
...
/*            Query = " SELECT a.PatientMRN, CONCAT(b.Title,' ',b.FirstName,' ',b.MiddleInitial,' ',b.LastName), " +
                    "DATE_FORMAT(b.DOB,'%d-%m-%Y'),  IFNULL(DATE_FORMAT(b.DateofService,'%m/%d/%Y %T')," +
                    "DATE_FORMAT(b.CreatedDate,'%m/%d/%Y %T')) AS DOS,  a.InvoiceNo, " +
                    "DATE_FORMAT(a.CreatedDate, '%m/%d/%Y %T'), a.TotalAmount, a.PaidAmount, a.BalAmount, a.Id,IFNULL(a.Discount,'0.00')  " +
                    "FROM " + Database + ".InvoiceMaster a  " +
                    "LEFT JOIN " + Database + ".PatientReg b ON a.PatientMRN = b.MRN   " +
                    "WHERE a.PatientMRN = '" + PatientInvoiceMRN + "' and a.Status = 0 ";*/
...
/*
            Query = "Select Id, MRN, IFNULL(FirstName,''), IFNULL(MiddleInitial,''), IFNULL(LastName,''), IFNULL(DOB,''), IFNULL(PhNumber,''),`Status`\n" +
                    "FROM " + Database + ".PatientReg \n" +
                    "WHERE status = 0 and CONCAT(FirstName,LastName,PhNumber,MRN,IFNULL(DATE_FORMAT(DOB,'%m-%d-%Y'),'')) like '%" + URLEncoder.encode(Patient, "UTF-8") + "%' ";
*/
            //System.out.println(Query);
