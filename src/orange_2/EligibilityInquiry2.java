...
//            }
    ...
    void GetResponse(final HttpServletRequest request, final PrintWriter out, final Connection conn, final ServletContext servletContext, final String UserId, final String Database, final int ClientId) {
        ...
        try {
            ...
            final String DOS = request.getParameter("DateofService").trim();
            final String PatientId = request.getParameter("PatientId").trim();
            ...
            finaltrizetto2.callSoapWebService(soapEndpointUrl, soapAction, GediPayerID, NPI, FirstName, LastName, InsuranceNum, DOB, Gender, GPN, PatientId, UserId, proname);
            ...
            final String strMsg = eiresponse2.finaloutput.toString();
            out.println(strMsg);
            try {
                final PreparedStatement MainReceipt = conn.prepareStatement("INSERT INTO oe.EligibilityInquiry (PatientMRN,DateofService,TraceId ,PolicyStatus,strmsg,Name, DateofBirth, Gender, InsuranceNum, GediPayerId, CreatedBy, CreatedDate) \nVALUES (?,?,?,?,?,?,?,?,?,?,?,now()) ");
